...
$operatorsStmt = $pdo->prepare(
    "SELECT admin_id, admin_firstname, admin_lastname, rank, conectat
      ...
      WHERE locatie = :locatie AND lucreaza_la = 'magazin'
        AND admin_id IN (SELECT admin_id FROM offline_online_operators)
...
function bestmixt_login_h($value): string
{
...
<head>
    ...
    <title>ECOGEST Bestmixt, conectare operator</title>
...
<main class="container login-shell">
    <header class="login-topbar">
        ...
        <nav class="login-toolbar" aria-label="Administrare locală">
            ...
            <form method="post" action="offline_users_sync.php" class="toolbar-form">
                <input type="hidden" name="csrf_token" value="<?= bestmixt_login_h($_SESSION['offline_login_csrf']) ?>">
    ...
    <?php if (isset($_GET['users_sync'])): ?>
        <div class="login-alert <?= $_GET['users_sync'] === 'success' ? 'is-success' : 'is-error' ?>" role="status">
            <?= bestmixt_login_h($_GET['message'] ?? ($_GET['users_sync'] === 'success'
    ...
    <section class="operators-panel">
        ...
        <div class="operators-grid">
            <?php foreach ($operators as $operator): ?>
                ...
                ?>
                <figure>
                    <button type="button" class="my_button" value="<?= $operatorId ?>" <?= $isConnected ? 'disabled' : '' ?> aria-label="Conectare <?= bestmixt_login_h($operatorName) ?>">
                        <img src="images/operator1.jpg" alt="">
                    </button>
                    <figcaption><?= bestmixt_login_h($operatorName !== '' ? $operatorName : ('Operator ' . $operatorId)) ?></figcaption>
                    <?php if ($isConnected): ?>
        ...
        <?php if (!empty($_SESSION['error'])): ?>
            <div class="login-alert is-error login-error"><?= bestmixt_login_h($_SESSION['error']) ?></div>
        ...
        function drainNext() {
            ...
            fetch('offline_sync_worker.php', {
                ...
                if (pending + sending > 0) {
                    finish('Confirmate acum: ' + sent + '. Coada continuă automat. Rămase: ' + (pending + sending + retry) + '.', false);
                    return;
                }
                if (blocked > 0) {
                ...
                if (retry > 0) {
                    finish((data.message ? data.message + ' ' : '') + 'Confirmate acum: ' + sent + '. În așteptare pentru reîncercare: ' + retry + '.', false);
                    return;
                }
