...
function opg_online_endpoint_reachable(string $url, float $timeoutSeconds = 1.2): bool
{
    $parts = parse_url($url);
...
function opg_extra_local_rows(PDO $pdo, string $table, string $pkColumn, array $onlineRows, int $previewLimit = 50): array
{
    $stats = [
        'extra_local' => 0,
        'preview_extra' => [],
    ...
    foreach (opg_fetch_table($pdo, $table, $pkColumn) as $localRow) {
        if (!is_array($localRow) || !array_key_exists($pkColumn, $localRow)) {
            continue;
        }

        $key = opg_lookup_key($localRow[$pkColumn], $pkType);
        if ($key === '' || isset($onlineKeys[$key])) {
            continue;
        }

        $stats['extra_local']++;
        if (count($stats['preview_extra']) < $previewLimit) {
            $preview = [
                'pk' => (string)$localRow[$pkColumn],
            ];
            if (array_key_exists('nume', $localRow)) {
                $preview['nume'] = (string)$localRow['nume'];
            }
            $stats['preview_extra'][] = $preview;
        }
    }
...
function opg_difference_stats(PDO $pdo, array $online, array $products): array
{
    ...
    foreach (['observatii_predefinite' => 'id', 'atribuiri_observatii_produse' => 'id_observatie'] as $table => $order) {
        $localRows = opg_fetch_table($pdo, $table, $order);
        $onlineRows = opg_filter_rows_for_hash($table, $online[$table] ?? []);
        $different = !hash_equals(opg_payload_hash([$table => $localRows]), opg_payload_hash([$table => $onlineRows]));
        $lookups[$table] = ['missing' => 0, 'different' => $different ? 1 : 0, 'received' => count($onlineRows)];
    }
    $lookupMissing = 0;
    $lookupDifferent = 0;
    ...
    $productStats = opg_compare_rows($pdo, 'produse_servicii', 'cod_produs', $onlineProducts);
    $productStats = array_merge(
        $productStats,
        opg_extra_local_rows($pdo, 'produse_servicii', 'cod_produs', $onlineProducts, 80)
    );
...
function opg_check_products_sync(PDO $pdo, array $restaurantConfig): array
{
    ...
    if (!opg_online_endpoint_reachable($config['api_url'])) {
        return [
            'allow' => true,
            'status' => 'offline',
            'message' => '',
    ...
    try {
        ...
        if ($changed) {
            ...
            if (!isset($onlineFull['observatii_predefinite'], $onlineFull['atribuiri_observatii_produse'])
                || !is_array($onlineFull['observatii_predefinite']) || !is_array($onlineFull['atribuiri_observatii_produse'])) {
                throw new RuntimeException('API-ul nu a trimis observațiile și atribuirile complete.');
            }
            ...
            $extraLocalProducts = (int)($diffStats['products']['extra_local'] ?? 0);
            if (!hash_equals($localCompatibleHash, $onlineCompatibleHash) || $extraLocalProducts > 0) {
                return [
                    'allow' => false,
