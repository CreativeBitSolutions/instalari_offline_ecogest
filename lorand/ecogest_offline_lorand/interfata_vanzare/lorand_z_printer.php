...
{
    if ($clientId !== 1019 || $reportNumber <= 0) {
        throw new InvalidArgumentException('Raportul de listare Lorand nu este valid.');
    }
    ...
    foreach ($contents as $type => $content) {
        ...
        try {
            ...
            $path = agecs_printer_enqueue_payload($clientId, $locationId, [
                'status' => 'success',
                'message' => 'Document de închidere Lorand pregătit pentru BAR.',
                'data' => [[
                    'id' => 0,
                    'data' => date('Y-m-d'),
                    'ora' => date('H:i:s'),
