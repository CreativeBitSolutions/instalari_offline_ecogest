...
if ($client_agecs == 16) {
    ...
    try {
        ...
        if ((int)$client_agecs === 1019 && $tip_plata === 'protocol') {
            $protocolRedirect = PAGINA_VANZARE;
            try {
                require_once __DIR__ . '/lorand_payment_note_printer.php';
                $lorandSettings = vanzare_v2_lorand_settings($pdo);
                if (!empty($lorandSettings['listare_nota_dupa_fiscalizare'])) {
                    ...
                    $protocolRedirect = 'asteapta_imprimanta.php?return=vanzare_magazin.php&context=nota_plata';
                }
            } catch (Throwable $e) {
                error_log('Eroare la listarea notei PROTOCOL Lorand (' . $nr_bon . '): ' . $e->getMessage());
                $_SESSION['offline_printer_error'] = 'Nota PROTOCOL este salvată, dar nu a putut fi pusă în coada imprimantei BAR: ' . $e->getMessage();
                $protocolRedirect = 'asteapta_imprimanta.php?return=vanzare_magazin.php&context=nota_plata';
            }
            unset($_SESSION['nr_bon'], $_SESSION['bon_procesat'], $_SESSION['numerarprim'], $_SESSION['cardprim'], $_SESSION['protocol'], $_SESSION['cif_client'], $_SESSION['rest_tichete'], $_SESSION['total_tichete'], $_SESSION['nota_noua'], $_SESSION['masa_curenta']);
            header('Location: ' . $protocolRedirect);
            exit();
        }
