...
function lorand_fiscal_signature(string $path): string
{
...
function lorand_fiscal_clear_wait(string $key): void
{
    unset($_SESSION['lorand_fiscal_wait'][$key]);
}

function lorand_fiscal_status(string $path, string $archive, string $key, int $maximumWait): array
{
    $signature = lorand_fiscal_signature($path);
    if ($signature === '') {
        lorand_fiscal_clear_wait($key);
        return ['pending' => false, 'archived' => false, 'remaining' => 0, 'elapsed' => 0];
    }

    $state = $_SESSION['lorand_fiscal_wait'][$key] ?? [];
    if (($state['signature'] ?? '') !== $signature) {
        $state = ['signature' => $signature, 'started' => time()];
        $_SESSION['lorand_fiscal_wait'][$key] = $state;
    }
    ...
    if (!is_dir($archive) && !@mkdir($archive, 0777, true) && !is_dir($archive)) {
        return [
            ...
            'remaining' => 0,
            'elapsed' => $elapsed,
            'error' => 'Bonul blocat nu a putut fi păstrat separat.',
        ];
    }

    $currentSignature = lorand_fiscal_signature($path);
    if ($currentSignature === '') {
        lorand_fiscal_clear_wait($key);
        return ['pending' => false, 'archived' => false, 'remaining' => 0, 'elapsed' => $elapsed];
    }
    if ($currentSignature !== $signature) {
        $_SESSION['lorand_fiscal_wait'][$key] = [
    ...
    $name = 'bon_neprocesat_' . date('Ymd_His') . '_' . substr($signature, 0, 10) . '.json';
    if (@rename($path, $archive . DIRECTORY_SEPARATOR . $name)) {
        lorand_fiscal_clear_wait($key);
        return ['pending' => false, 'archived' => true, 'remaining' => 0, 'elapsed' => $elapsed];
    }
    if (!is_file($path)) {
        lorand_fiscal_clear_wait($key);
        return ['pending' => false, 'archived' => false, 'remaining' => 0, 'elapsed' => $elapsed];
...
function lorand_fiscal_skip_pending(string $queuePath, string $archiveDirectory, string $waitKey): array
{
    $queueHelper = lorand_offline_api_root() . DIRECTORY_SEPARATOR . 'printer_queue_atomic_helper.php';
    ...
    if (($claim['status'] ?? '') === 'empty') {
        lorand_fiscal_clear_wait($waitKey);
...
if (isset($_GET['status'])) {
    ...
    echo json_encode(
        lorand_fiscal_status($queuePath, $archiveDirectory, $waitKey, $maximumWait),
        JSON_UNESCAPED_UNICODE
    );
    exit;
}

$afterUrl = trim((string)($_SESSION['lorand_fiscal_after_url'] ?? ''));
$generator = 'casa_marcat_vanzare.php';
$directFiscalEnabled = $clientId === 1019 && $locationId > 0;
if ($directFiscalEnabled && empty($_SESSION['lorand_direct_fiscal_csrf'])) {
    try {
        $_SESSION['lorand_direct_fiscal_csrf'] = bin2hex(random_bytes(24));
    } catch (Throwable $error) {
        $_SESSION['lorand_direct_fiscal_csrf'] = hash('sha256', session_id() . microtime(true));
    }
}
$directFiscalToken = $directFiscalEnabled
    ? (string)($_SESSION['lorand_direct_fiscal_csrf'] ?? '')
    : '';
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST'
    ...
    if (!$directFiscalEnabled
        ...
        echo json_encode(['success' => false, 'message' => 'Operația este disponibilă numai în instalarea locală Lorand.'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    try {
        $result = lorand_fiscal_skip_pending($queuePath, $archiveDirectory, $waitKey);
        $nextUrl = $afterUrl !== '' ? $afterUrl : $generator;
        unset($_SESSION['lorand_fiscal_after_url']);
...
(function(){
    ...
    function complete(message,delay){
        ...
        title.textContent='Bonul a fost trimis către casa de marcat';
    ...
    function directFiscal(){
        ...
        fetch('trimitere_fiscala_directa_lorand.php',{
            method:'POST',
            cache:'no-store',
