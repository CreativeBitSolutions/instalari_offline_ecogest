<?php
   ...
   $data_start=$_SESSION['data_start_intrari_in_gest'];
        $data_end=$_SESSION['data_end_intrari_in_gest'];
?>
<?php
...
$this->Cell(115	,10,'Intrari '.$gest.' de la '.date("d-m-Y", strtotime($_SESSION['data_start_intrari_in_gest'])).' pana la '.date("d-m-Y", strtotime($_SESSION['data_end_intrari_in_gest'])),1,1,C);
...
while ($row = $f_stmt->fetch(PDO::FETCH_ASSOC)){ 
    ...
    $luna=$row['luna']; 
    $anul=$row['anul'];
    $valoare=round($row['valoare'],4);
    $pdf->Cell(60	,10,$ziua.'-'.$luna.'-'.$anul,1,0,C);
$pdf->Cell(55	,10,$valoare,'1',1,C);//end of line
