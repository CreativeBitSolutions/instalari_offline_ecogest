...
function offline_export_build(PDO $pdo, $dataStartRaw, $dataEndRaw, $formatRaw = 'xml')
{
    ...
    $clientId = (int)offline_export_app_config_value('client_id', 21);
    ...
    if ($format === 'sql') {
        $output .= "-- Export offline vanzari La Voinica\n";
    ...
    $ext = $format;
    $filename = "export_offline_la_voinica_{$data_start}_{$data_end}." . $ext;
