...
<li class="breadcrumb-item active">Documente</li>
	 ...
	 <div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-table"></i> Lista Note Hotel</div>
        <div class="card-body">
          <div class="table-responsive">
<?php

$sql = "SELECT * from $tabel_final_check_in inner join $tabel_final_admins on $tabel_final_check_in.operator = $tabel_final_admins.admin_id ";    
...
$modif_suma=$row['id_c'];
$modif_suma=strval($modif_suma);
$modif_suma.='MS';

$firstname=$row["admin_firstname"];
$lastname=$row["admin_lastname"];
echo"<tr>";    
echo"<td>".$row['id_c']."</td><td>".$row['data_incasarii']."    ".$row['ora_bon']."</td><td>".$firstname." ".$lastname. "</td>";
...
if($canti_total_stornata<$cantitate_totala_bon){
              echo "<form method='post'><input class='btn btn-primary btn-block' type='submit' value='Stornare Nota Hotel' name='$storn_bon'></form>";
}
...
echo "</td><td><form method='post'><input class='btn btn-primary btn-block' type='submit' value='Document Stornare' name='$list_storn'></td></form>";
  echo "</td><td><form method='post'><input class='btn btn-primary btn-block' type='submit' value='Modificare sume' name='$modif_suma'></td></form>";
