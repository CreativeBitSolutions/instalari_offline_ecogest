...
$GLOBALS['sync_api_columns_cache'] = [];
...
function sync_api_connect_client_database(array $client)
{
    ...
    if ($db === '' || $user === '') {
        sync_api_json(500, [
            'status' => 'error',
            'message' => 'Datele de conectare ale clientului sunt incomplete.',
        ]);
    }

    return sync_api_pdo("mysql:host=localhost;dbname={$db};charset=utf8mb4", $user, $pass);
}

function sync_api_quote_identifier($identifier)
{
...
function sync_api_table_exists(PDO $pdo, $table)
{
    $stmt = $pdo->prepare("
        SELECT COUNT(*)
...
function sync_api_get_columns(PDO $pdo, $table)
{
    if (isset($GLOBALS['sync_api_columns_cache'][$table])) {
        return $GLOBALS['sync_api_columns_cache'][$table];
    }

    if (!sync_api_table_exists($pdo, $table)) {
        $GLOBALS['sync_api_columns_cache'][$table] = [];
        return [];
    }

    $stmt = $pdo->query('SHOW FULL COLUMNS FROM ' . sync_api_quote_identifier($table));
    $columns = [];
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $columns[$row['Field']] = $row;
    }

    $GLOBALS['sync_api_columns_cache'][$table] = $columns;
    return $columns;
}

function sync_api_products_table_columns($table)
{
    $columns = [
        'produse_servicii' => [
            'cod_produs',
...
function sync_api_products_hash_column_type($column)
{
    $integerColumns = [
        'cod_produs',
        'id_categorie',
...
function sync_api_products_hash_value($value, $column)
{
    if ($value === null) {
        return null;
    }

    $type = sync_api_products_hash_column_type($column);
    if ($type === 'int') {
...
function sync_api_products_normalize_for_hash(array $payload)
{
    foreach ($payload as $key => $rows) {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($row as $column => $value) {
                $row[$column] = sync_api_products_hash_value($value, (string)$column);
            }
            ksort($row);
            $normalizedRows[] = $row;
        }
...
function sync_api_products_payload_hash(array $payload)
{
    $json = json_encode(sync_api_products_normalize_for_hash($payload), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    if ($json === false) {
...
function sync_api_products_fetch_table(PDO $pdo, $table, array $orderCandidates)
{
    if (!sync_api_table_exists($pdo, $table)) {
        return [
            'columns' => [],
    ...
    $dbColumns = sync_api_get_columns($pdo, $table);
    $syncColumns = sync_api_products_table_columns($table);
    $columns = [];
    ...
    foreach ($columns as $column) {
        $quotedColumns[] = sync_api_quote_identifier($column);
    }

    $sql = 'SELECT ' . implode(', ', $quotedColumns) . ' FROM ' . sync_api_quote_identifier($table);
    if ($orderColumn !== '') {
        $sql .= ' ORDER BY ' . sync_api_quote_identifier($orderColumn);
    }
...
function sync_api_products_validate_requested_client(array $client)
{
    ...
    if ($requestedClientId > 0 && $requestedClientId !== $clientId) {
        sync_api_json(403, [
            'status' => 'error',
            'message' => 'cod_client nu corespunde cu API key.',
        ]);
    }
}

function sync_api_products_send_export(PDO $pdo, array $client)
{
    sync_api_products_validate_requested_client($client);

    $products = sync_api_products_fetch_table($pdo, 'produse_servicii', ['cod_produs']);
    if (!in_array('cod_produs', $products['columns'], true)) {
        sync_api_json(500, [
            'status' => 'error',
            'message' => 'Tabela produse_servicii nu contine cod_produs.',
        ]);
    }

    $categorii = sync_api_products_fetch_table($pdo, 'categorii', ['id_categorie']);
    $categoriiLocatii = sync_api_products_fetch_table($pdo, 'categorii_locatii', ['id']);
    $gestiuni = sync_api_products_fetch_table($pdo, 'gestiuni', ['id_gestiune']);
    $coteTva = sync_api_products_fetch_table($pdo, 'cote_tva', ['cota']);
    ...
    $productsHash = sync_api_products_payload_hash($hashPayload);
    ...
    $response = [
        'status' => 'success',
        'schema_version' => OFFLINE_PRODUCTS_SCHEMA_VERSION,
    ...
    if ($includeData) {
        ...
        $response['categorii'] = $categorii['data'];
        $response['categorii_locatii'] = $categoriiLocatii['data'];
        $response['gestiuni'] = $gestiuni['data'];
        $response['cote_tva'] = $coteTva['data'];
        $response['lookup_counts'] = [
            'categorii' => count($categorii['data']),
            'categorii_locatii' => count($categoriiLocatii['data']),
            'gestiuni' => count($gestiuni['data']),
            'cote_tva' => count($coteTva['data']),
        ];
        $response['hash_payload_tables'] = array_keys($hashPayload);
    }

    sync_api_json(200, $response);
}

if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $client = sync_api_auth_client([]);
    $pdo = sync_api_connect_client_database($client);
    sync_api_products_send_export($pdo, $client);
}
