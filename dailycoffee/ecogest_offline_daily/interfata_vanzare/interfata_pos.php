...
include('database_connection.php'); // definește $pdo, $tabel_final_det_note, $tabel_final_nomenclator, $tabel_final_note
...
// Citește și stochează nr_bon în sesiune doar o singură dată
...
// Preluăm nr_bon din sesiune
$nr_bon_curent = $_SESSION['nr_bon_curent'] ?? null;
if (!$nr_bon_curent) {
    echo "<p class='text-center mt-5'><strong>Nu există un bon deschis în acest moment.</strong></p>";
    exit;
}
...
// Preluăm antetul (numerar, rest, cif_client)
$antet = $pdo->prepare("
...
// ENDPOINT AJAX „CHECK” (timestamp-ul JSON-ului)
...
// ENDPOINT AJAX „LOAD” (structura tabelului)
...
// Funcția PHP care generează HTML-ul tabelului
function afiseaza_tabel($pdo, $nr_bon, $tabel_det, $tabel_nom, $tabel_produse_serv, $tabel_note, $suma_incasata, $rest_rachitat, $cif_client) {
    ...
    if (count($rows) === 0) {
        echo "<p class='text-center py-4'><em>Bonul nu conține produse.</em></p>";
        return;
    }
    ?>
    <table class="table table-hover align-middle mb-0">
      <thead class="table-dark">
        <tr>
          ...
          <th scope="col">Preț vânzare<br>(unitar)</th>
          <th scope="col">Total<br>vânzare</th>
        </tr>
      </thead>
      <tbody>
        <?php
          ...
          foreach ($rows as $row) {
              ...
              if ($imagine === '' || !file_exists($imagine)) {
                  $imagineSrc = 'https://via.placeholder.com/60x60?text=Produs';
              } else {
                  $imagineSrc = $imagine;
              }
        ?>
        ...
        <tr class="summary-row">
          <td colspan="4" class="text-end">Total de încasat:</td>
    ...
    <div class="col-12 col-lg-10">
      <div class="card mb-4">
        <div class="card-header bg-white border-bottom">
          <h3 class="mb-0">Comanda dumneavoastră</h3>
        </div>
        <div class="card-body">
          <div id="prod-container">
            <?php
            // Afișăm tabelul complet, generat de PHP
            afiseaza_tabel(
                $pdo,
                $nr_bon_curent,
                $tabel_det,
...
<script>
  ...
  function incarcaTabel() {
    fetch('?ajax=1')
      ...
      .catch(err => console.error('Eroare la încărcarea tabelului:', err));
  }
  ...
  function verificaProduseNoi() {
    fetch('?ajax_check=1')
      ...
      .catch(err => console.error('Eroare la verificarea modificărilor:', err));
  }

  document.addEventListener('DOMContentLoaded', () => {
    // inițializăm timestamp și pornim polling-ul
    verificaProduseNoi();
