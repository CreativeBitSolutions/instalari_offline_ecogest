...
function offline_cui_client_response($status, array $payload)
{
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    offline_cui_client_response(405, array('success' => false, 'message' => 'Metoda nepermisa.'));
}
if (empty($_SESSION['admin_id'])) {
    offline_cui_client_response(401, array('success' => false, 'message' => 'Sesiunea operatorului a expirat.'));
}
$csrf = isset($_POST['csrf_token']) ? (string)$_POST['csrf_token'] : '';
if (empty($_SESSION['offline_cui_csrf']) || !hash_equals((string)$_SESSION['offline_cui_csrf'], $csrf)) {
    offline_cui_client_response(403, array('success' => false, 'message' => 'Cererea de verificare nu este valida.'));
}
...
if ($action === 'clear') {
    ...
    offline_cui_client_response(200, array('success' => true, 'cif_client' => '', 'verified_online' => false));
}
if ($cui === '' || !offline_cui_is_valid_local($cui)) {
    offline_cui_client_response(422, array('success' => false, 'code' => 'invalid_cui', 'message' => 'CUI-ul introdus nu este valid.'));
}

if ($action === 'lookup') {
    $result = offline_cui_remote_lookup(offline_config_all(), $cui);
    if (empty($result['ok']) || empty($result['data']) || !is_array($result['data'])) {
        offline_cui_client_response(
    ...
    if (offline_cui_digits_local($confirmed) !== offline_cui_digits_local($cui)) {
        offline_cui_client_response(502, array('success' => false, 'message' => 'Datele firmei nu confirma CUI-ul introdus.', 'manual_allowed' => true));
    }
    ...
    offline_cui_client_response(200, array(
        'success' => true,
        'code' => 'company_found',
...
if ($action === 'save_verified') {
    ...
    if (!$validLookup) {
        offline_cui_client_response(409, array('success' => false, 'message' => 'Verificarea firmei a expirat. Verificati din nou CUI-ul.'));
    }
    ...
    unset($_SESSION['offline_cui_lookup']);
    offline_cui_client_response(200, array('success' => true, 'cif_client' => $savedCui, 'verified_online' => true));
}
...
if ($action === 'save_manual') {
    ...
    offline_cui_client_response(200, array('success' => true, 'cif_client' => $cui, 'verified_online' => false));
}

offline_cui_client_response(400, array('success' => false, 'message' => 'Actiunea solicitata nu este valida.'));
