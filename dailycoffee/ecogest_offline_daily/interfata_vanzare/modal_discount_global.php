...
$cod_locatie_discount = (int)($_SESSION['cod_locatie'] ?? 2);
...
if (isset($_POST['apl_disc_fix_global'])) {
    ...
    foreach ($all_products as $prod) {
        if (strpos($prod['nume_produs'], 'SGR') === false) {
    ...
    try {
        ...
        foreach ($non_sgr_products as $prod) {
            ...
            // NOU: Inserare audit
            $stmt_insert = $pdo->prepare("INSERT INTO discounturi_acordate (id_vanz, id_operator, cod_locatie, tip_discount, valoare_discount_ron, pret_unitar_initial, pret_unitar_final, id_operatiune_globala) VALUES (?, ?, ?, 'global_valoric', ?, ?, ?, ?)");
            $stmt_insert->execute([$id_vanz, $adm_id, $cod_locatie_discount, $discount_for_row, $pret_unitar_initial, $pret_unitar_final, $id_operatiune_globala]);
...
if (isset($_POST['apl_disc_proc_global'])) {
    ...
    $grand_total = 0; $total_non_sgr = 0; $lista_produse_non_sgr = [];
    foreach ($all_products as $row) {
        $line_value = round($row['pret_vanzare'] * $row['cantitate'], 2);
        $grand_total += $line_value;
        if (strpos($row['nume_produs'], 'SGR') === false) {
    ...
    if ($grand_total > 0 && $procent > 0) {
        $pdo->beginTransaction();
        try {
            $discount_total = round(($grand_total * $procent / 100), 2);
            ...
            for ($i = 0; $i < count($lista_produse_non_sgr); $i++) {
                ...
                $stmt_insert = $pdo->prepare("INSERT INTO discounturi_acordate (id_vanz, id_operator, cod_locatie, tip_discount, valoare_procent, valoare_discount_ron, pret_unitar_initial, pret_unitar_final, id_operatiune_globala) VALUES (?, ?, ?, 'global_procentual', ?, ?, ?, ?, ?)");
                $stmt_insert->execute([$id_vanz, $adm_id, $cod_locatie_discount, $procent, $discount_pe_linie, $pret_unitar_initial, $pret_unitar_final, $id_operatiune_globala]);
    ...
    if ($products_to_reset) {
        ...
        try {
            foreach ($products_to_reset as $row) {
                ...
                $stmt_insert = $pdo->prepare("INSERT INTO discounturi_acordate (id_vanz, id_operator, cod_locatie, tip_discount, valoare_discount_ron, pret_unitar_initial, pret_unitar_final, id_operatiune_globala) VALUES (?, ?, ?, 'resetare', ?, ?, ?, ?)");
                $stmt_insert->execute([$id_vanz, $adm_id, $cod_locatie_discount, $valoare_modificata_ron, $pret_unitar_curent, $pret_unitar_resetat, $id_operatiune_globala]);
