...
if(!isset($GLOBALS['casa_after_write'])&&(($_SERVER['REQUEST_METHOD']??'GET')==='POST'||in_array(basename($_SERVER['SCRIPT_NAME']??''),['factura.php','duplicare_factura_corectare.php'],true))){
    ...
    register_shutdown_function(function(){
        ...
        try {
            ...
            require_once __DIR__.'/offline_sync.php';casa_sync_tick();
...
if(in_array(basename($_SERVER['SCRIPT_NAME']??''),$casaEditScripts,true)){
    ...
    if(basename($_SERVER['SCRIPT_NAME']??'')==='stergere_factura.php'&&(int)($meta['revision']??0)>0)casa_json(['success'=>false,'error'=>'Factura a intrat deja în sincronizare. Ștergerea se gestionează online.'],409);
    if(!$meta||$meta['state']!=='draft'||$meta['origin']==='historical')casa_json(['success'=>false,'error'=>'Factura este finalizată sau aparține istoricului. Redeschideți factura locală din Sincronizare înainte de modificare.'],409);
}
