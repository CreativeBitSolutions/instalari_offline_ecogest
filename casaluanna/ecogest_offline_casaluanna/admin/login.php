...
?><!doctype html><html lang="ro"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="casa-csrf" content="<?=casa_h($_SESSION['casa_csrf'])?>"><title>Casa Luanna, autentificare</title><link rel="stylesheet" href="vendor/offline/bootstrap4/bootstrap.min.css"><script src="offline_ui.js" defer></script></head><body class="bg-light"><main class="card p-4 mx-auto mt-5" style="max-width:460px"><h1 class="h3">ECOGEST · Casa Luanna</h1><p>Facturare offline</p><form method="post"><input type="hidden" name="casa_csrf" value="<?=casa_h($_SESSION['casa_csrf'])?>"><label>Adresa de email</label><input class="form-control mb-3" type="email" name="email_address" required autocomplete="username"><label>Parola</label><input class="form-control mb-3" type="password" name="password" required autocomplete="current-password"><button class="btn btn-primary">Autentificare</button></form><button type="button" id="pull-users" class="btn btn-outline-primary mt-3">Preia utilizatorii din online</button><p id="pull-users-message" role="status"></p><p class="text-danger mt-3"><?=casa_h($error)?></p><small id="casa-sync">Sincronizare automată</small></main><script>
document.getElementById('pull-users').onclick=async function(){
 const button=this,message=document.getElementById('pull-users-message');button.disabled=true;message.textContent='Se preiau utilizatorii...';
 try{const response=await fetch('pull_online.php',{method:'POST',headers:{'X-Casa-CSRF':document.querySelector('meta[name="casa-csrf"]').content},body:new URLSearchParams({action:'users'})});const data=await response.json();if(!data.success)throw new Error(data.error);message.textContent='Utilizatorii au fost actualizați. Vă puteți autentifica.';}
 catch(e){message.textContent=e.message||'Preluarea nu a fost confirmată.';}finally{button.disabled=false;}
};
</script></body></html>
