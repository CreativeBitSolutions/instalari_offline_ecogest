...
set_exception_handler(function(Throwable $error) use($pdo){
    if($pdo->inTransaction())$pdo->rollBack();
    error_log('Casa factura produs: '.$error->getMessage());
    casa_json(['success'=>false,'error'=>'Produsul nu a fost salvat. Verificați nomenclatorul și jurnalul local.'],422);
});
...
$del_stmt->execute([':id_factura' => $id_factura]);
                ...
                // Răspunsul se trimite după commit.
        ...
        $completeLine=casa_one($pdo,'SELECT * FROM vanzari WHERE id_vanz=?',[$ultim_id_vanz_inserat]);
        $cantitate=(float)$completeLine['cantitate'];
        $gross=round((float)$completeLine['pret_vanzare']*$cantitate-(float)$completeLine['discount'],2);
        $tax=round($gross*$cota_tva/(100+$cota_tva),2);
        $pdo->prepare('UPDATE vanzari SET valoare_vanzare_cu_tva=?,tva_col=?,valoare_vanzare=? WHERE id_vanz=?')->execute([$gross,$tax,$gross-$tax,$ultim_id_vanz_inserat]);
        $pdo->prepare('DELETE FROM miscari WHERE id_vanz_fact=?')->execute([$ultim_id_vanz_inserat]);
...
";
        ...
        if(!$produs_data)throw new RuntimeException('Produs fără gestiune în nomenclator.');
