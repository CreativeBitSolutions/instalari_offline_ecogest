...
function casa_delete_local(PDO $db,int $id): void {
    ...
    foreach(['casa_fiscal_jobs','casa_fiscal_attempts'] as $table)if(casa_one($db,"SELECT name FROM sqlite_master WHERE type='table' AND name=?",[$table]))$archive[$table]=casa_rows($db,'SELECT * FROM '.$table.' WHERE id_factura=?',[$id]);
    ...
    foreach(['casa_fiscal_jobs','casa_fiscal_attempts'] as $table)if(casa_one($db,"SELECT name FROM sqlite_master WHERE type='table' AND name=?",[$table]))$db->prepare('DELETE FROM '.$table.' WHERE id_factura=?')->execute([$id]);
...
function casa_request_delete(PDO $db,int $id): void {
    ...
    if(($meta['state']??'')==='remote_deleted'||!empty($status['online_deleted'])||($status['lifecycle']??'')==='deleted'){
        casa_delete_local($db,$id);return;
    }
