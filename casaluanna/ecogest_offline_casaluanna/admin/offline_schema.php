...
function casa_schema(PDO $db): void {
    ...
    try{
        ...
        if(!in_array('finalized',$columns,true))$db->exec("ALTER TABLE casa_invoices ADD COLUMN finalized INTEGER NOT NULL DEFAULT 0");
        ...
        foreach(['vanzari'=>'id_vanz','incasari'=>'id_incasare','miscari'=>'id','stornari'=>'id_stornare'] as $table=>$pk){
            ...
            $db->exec("INSERT OR IGNORE INTO casa_remote_rows SELECT '$table',t.$pk,t.$pk,c.id_factura FROM $table t $join WHERE c.origin='historical'");
        }
        $db->exec("INSERT OR REPLACE INTO casa_meta(name,value) VALUES('schema_version','3')");$db->commit();
