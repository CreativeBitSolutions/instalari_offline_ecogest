...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id_vanz > 0 && $cantitate > 0) {
        ...
        $select_sql = "SELECT pret_vanzare, cota_tva, id_factura, discount, cantitate FROM vanzari WHERE id_vanz = :id_vanz";
        ...
        if ($product = $select_stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $valoare_vanzare_cu_tva = round($pret_vanzare * $cantitate - (float)$product['discount'],2);
     ...
     SET cantitate_misc = cantitate_misc * :cantitate_misc 
     WHERE id_vanz_fact = :id_vanz";
try {
$stmt_miscari_update = $pdo->prepare($psql_miscari_update);
$stmt_miscari_update->execute([
':cantitate_misc' => (float)$product['cantitate'] != 0 ? $cantitate/(float)$product['cantitate'] : 0,
':id_vanz'        => $id_vanz
]);
