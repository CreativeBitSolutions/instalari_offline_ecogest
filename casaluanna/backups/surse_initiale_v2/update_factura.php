...
if (trim((string)$serie_factura) !== '') {
    ...
    if (!$este_acelasi_numar_deja_salvat) {
        $max_nr_sql = "SELECT MAX(CAST(nr_factura AS UNSIGNED)) AS max_nr
                       FROM facturi
        ...
        if ($max_nr > 0) {
            ...
            $nr_inceput_sql = "SELECT nr_inceput FROM serii_documente WHERE serie = :serie_factura LIMIT 1";
...
function getClientColumns($pdo) {
    static $columns = null;
    if ($columns === null) {
        $stmt = $pdo->prepare("DESCRIBE clienti");
        $stmt->execute();
        $columns = [];
