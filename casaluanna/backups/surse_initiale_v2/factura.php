...
$tva_col = round($valoare_vanzare_cu_tva - ($valoare_vanzare_cu_tva / (1 + $cota_tva / 100)), 2);
...
try {
 ...
 error_log("Eroare la recalcularea vanzarii ID $id_vanz: " . $e->getMessage());
  ...
  try {
    ...
    $update_um_sql = "
        UPDATE vanzari
        JOIN produse_servicii ON vanzari.den_p = produse_servicii.nume
        SET vanzari.um = produse_servicii.um
        WHERE vanzari.id_factura = :id_factura
    ";
...
try {
    $ef_stmt = $pdo->query("SELECT email FROM date_firma LIMIT 1");
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-12 mb-3">
                            ...
                            <select name="serie_factura" id="serie_factura" class="form-control" required>
                                ...
                                $series_sql = "SELECT serie FROM serii_documente WHERE tip_registru LIKE '%factura%'";
...
<td><input class="form-control virtual-keyboard" type="number" name="cantitate" value="1" step="0.00001" required></td>
    ...
    <!-- Buton de validare ANAF -->
    <?php if (!$is_validated): ?>
        <a href="anaf_validare.php?id_factura=<?php echo urlencode($id_factura); ?>"
           class="btn btn-primary btn-block <?= $hideStep2 ?>"
           onclick="return confirm('Ești sigur că vrei să validezi această factură la ANAF?');">
          Validează la ANAF
        </a>
    <?php else: ?>
        <button class="btn btn-success btn-block <?= $hideStep2 ?>" disabled>
            Factură Validată la ANAF
        </button>
        <p class="<?= $hideStep2 ?>"><strong>Data Validării:</strong> <?php echo htmlspecialchars($validation_timestamp); ?></p>
        <a href="istoric_validari.php?id_factura=<?php echo urlencode($id_factura); ?>" class="btn btn-info btn-block <?= $hideStep2 ?>">
            Vezi Istoricul Validărilor
        </a>
        <a href="sterge_validari.php?id_factura=<?php echo urlencode($id_factura); ?>"
           class="btn btn-danger btn-block <?= $hideStep2 ?>"
    ...
    <?php if ($is_validated && !$is_incarcat): ?>
        <a href="anaf_incarcare.php?id_factura=<?php echo urlencode($id_factura); ?>"
           class="btn btn-warning btn-block <?= $hideStep2 ?>"
           onclick="return confirm('Ești sigur că vrei să încarci această factură la ANAF?');">
          Încarcă la ANAF
        </a>
       
    <?php elseif (!$is_validated): ?>
        <div class="alert alert-warning <?= $hideStep2 ?>">
            Factura nu este validată. Te rog să validezi factura mai întâi.
        </div>
    <?php elseif ($is_validated && $is_incarcat): ?>
        <button class="btn btn-secondary btn-block <?= $hideStep2 ?>" disabled>
            Factură Încărcată la ANAF
        </button>
        <p class="<?= $hideStep2 ?>"><strong>Index Încărcare:</strong> <?php echo htmlspecialchars($incarcare_index); ?></p>
        <a href="istoric_incarcari.php?nr_factura=<?php echo urlencode($nr_factura); ?>&serie_factura=<?php echo urlencode($serie_factura); ?>" class="btn btn-info btn-block <?= $hideStep2 ?>">
            Vezi Istoricul Încărcărilor
        </a>
    <?php endif; ?>

    <?php if (!$is_incarcat): ?>
        <a href="anaf_xml_genereaza.php?id_factura=<?php echo urlencode($id_factura); ?>"
           class="btn btn-outline-primary btn-block <?= $hideStep2 ?>">
           Creeaza XML ANAF
        </a>
        <?php if (!empty($anaf_xml_pregatit['id'])): ?>
            <a href="anaf_xml_previzualizare.php?id=<?php echo urlencode((string)$anaf_xml_pregatit['id']); ?>"
               class="btn btn-outline-info btn-block <?= $hideStep2 ?>">
               Vezi XML pregatit pentru incarcare directa
            </a>
        <?php endif; ?>
    <?php endif; ?>
