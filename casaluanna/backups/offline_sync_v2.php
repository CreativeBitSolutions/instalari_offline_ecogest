...
function casa_snapshot(PDO $db,int $id): array {
    ...
    return ['factura'=>$invoice,'vanzari'=>$rows,'client'=>$client,
        'incasari'=>casa_rows($db,'SELECT * FROM incasari WHERE id_factura=? ORDER BY id_incasare',[$id]),
        'miscari'=>casa_rows($db,'SELECT m.* FROM miscari m JOIN vanzari v ON v.id_vanz=m.id_vanz_fact WHERE v.id_factura=? ORDER BY m.id_miscare',[$id]),
...
function casa_enqueue(PDO $db,int $id): bool {
    ...
    if(!$meta||$meta['state']==='historical')throw new RuntimeException('Factura istorică se gestionează în aplicația online.');
