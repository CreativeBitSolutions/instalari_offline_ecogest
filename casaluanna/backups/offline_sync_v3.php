...
function casa_snapshot(PDO $db,int $id): array {
    ...
    $meta=casa_one($db,'SELECT origin,online_id FROM casa_invoices WHERE id_factura=?',[$id]);
    if(($meta['origin']??'')==='historical'){
        $baseline=(int)$db->query("SELECT value FROM casa_meta WHERE name='receipt_baseline'")->fetchColumn();
        $receipts=casa_rows($db,'SELECT * FROM incasari WHERE id_factura=? AND id_incasare>? ORDER BY id_incasare',[$id,$baseline]);
    ...
    if((int)$invoice['nr_factura']<1||trim((string)$invoice['serie_factura'])===''||!$rows)throw new RuntimeException('Completați seria, numărul și produsele facturii.');
    ...
    foreach($stornari as &$storno){
        ...
        if(!$ref)throw new RuntimeException('Factura stornată nu a fost găsită.');$storno['offline_reference']=$ref;
    }unset($storno);
    return ['factura'=>$invoice,'vanzari'=>$rows,'client'=>$client,'stornari'=>$stornari,
...
function casa_enqueue(PDO $db,int $id): bool {
    ...
    if($hash===$meta['hash']){if($meta['state']==='draft')$db->prepare("UPDATE casa_invoices SET state='synced' WHERE id_factura=?")->execute([$id]);return false;}
    ...
    $db->prepare("UPDATE casa_invoices SET state='queued',revision=?,hash=?,error='' WHERE id_factura=?")->execute([$revision,$hash,$id]);
    return true;
}
function casa_sync_tick(): array {
    ...
    try{
        ...
        try{
            foreach(casa_rows($db,"SELECT id_factura FROM casa_invoices WHERE state='synced'") as $row){
                $db->beginTransaction();try{casa_enqueue($db,(int)$row['id_factura']);$db->commit();}catch(Throwable $e){$db->rollBack();throw $e;}
            }
        ...
        if($code>=200&&$code<300&&is_array($reply)&&($reply['status']??'')==='success'&&($reply['event_uuid']??'')===$event['event_uuid']&&($reply['content_hash']??'')===$event['hash']&&(int)($reply['online_id']??0)>0){
            ...
            try{
                ...
                $db->prepare("UPDATE casa_invoices SET state='synced',online_id=?,error='' WHERE id_factura=? AND revision=? AND state='queued'")->execute([(int)$reply['online_id'],(int)$event['id_factura'],(int)$event['revision']]);$db->commit();
