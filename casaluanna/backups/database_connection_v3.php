...
if(!isset($GLOBALS['casa_write_lock'])&& (($_SERVER['REQUEST_METHOD']??'GET')==='POST'||basename($_SERVER['SCRIPT_NAME']??'')==='factura.php')){
...
if(in_array(basename($_SERVER['SCRIPT_NAME']??''),$casaEditScripts,true)){
    ...
    $meta=casa_one($pdo,'SELECT state,origin FROM casa_invoices WHERE id_factura=?',[$editId]);
