<?php // casa_marcat_vanzare_cu_rest.php
...
// === ADAUGAT: Variabila pentru calcul total produse ===
    $total_bon_calculat = 0;
    ...
    while ($row = $f_stmt->fetch(PDO::FETCH_ASSOC)) {
      ...
      // === ADAUGAT: Adunam la totalul bonului pentru comparatie ulterioara ===
        $total_bon_calculat += ($pret_vanzare_fin * $cantitate);
        // ======================================================================
    ...
    if ($notaRow) {
        // === MODIFICAT PENTRU REST NUMERAR ===
        // === MODIFICARE PENTRU REST ===
        if (isset($notaRow['numerar']) && $notaRow['numerar'] > 0) {
            $suma_finala_de_trimis = $notaRow['numerar']; // Default: cat e pe bon

            // Verificam variabila noastra explicita
            if (isset($_SESSION['baniprimiti'])) {
                $bani_primiti = (float)$_SESSION['baniprimiti'];
                $total_produse = (float)$total_bon_calculat;

                // Daca banii primiti sunt mai multi decat valoarea produselor, trimitem banii primiti
                // Casa va calcula restul automat.
                // Folosim o marja mica (0.001) pentru diferente de float
                if ($bani_primiti > $total_produse + 0.001) {
                    $suma_finala_de_trimis = $bani_primiti;
                }
            }
            
            // Scriem linia T cu suma decisa
            $myBuffer .= $T . "0;" . number_format($suma_finala_de_trimis, 2, '.', '') . ";;;;" . $cr;
        }
        // =====================================
