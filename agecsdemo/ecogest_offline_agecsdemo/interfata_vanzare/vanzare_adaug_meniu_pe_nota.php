...
include('session.php');
if (!in_array((int)($_SESSION['client_id'] ?? 0), [17])) {
    http_response_code(403);
    echo json_encode(['error' => 'FuncÈ›ionalitate indisponibilÄƒ pentru acest client.']);
    exit;
}
...
try {
  ...
  if (!$nr_bon || !$cod_masa || !$cod_meniu) {
    throw new Exception('Parametri lipsÄƒ.');
  }

  // citim conÈ›inut + detalii din nomenclator pentru TVA, UM, SGR, gestiune
  $sql = "
  ...
  // funcÈ›ie localÄƒ SGR (identicÄƒ semantic cu cea din vanzare_adaug_prod_pe_nota.php)
  function adaugaGarantieSGR($cod_garantie, $cant, $nr_bon, $pachet, $pdo, $tabel_final_nomenclator, $tabel_final_det_note, array &$resp) {
    ...
    if ($g = $stmt->fetch(PDO::FETCH_ASSOC)) {
      ...
      $ins = $pdo->prepare("
        INSERT INTO {$tabel_final_det_note}
          (nr_bon,cod_p,nume_produs,cantitate,cota_tva,tva_col,pret_vanzare,valoare_vanzare,valoare_vanzare_cu_tva,pachet,data,ora)
        VALUES (:nr,:cod,:nume,:cant,:cota,:tva,:pret,:val,:val_cu,:pachet,date('now','localtime'),time('now','localtime'))
      ");
      $ins->execute([
  ...
  // setÄƒri tabele din mediul tÄƒu
  ...
  foreach ($items as $it) {
    ...
    // cantitatea finalÄƒ per linie = cantitatea din meniu * numÄƒrul de meniuri
    ...
    // dacÄƒ existÄƒ linie identicÄƒ (acelaÈ™i cod + preÈ› + pachet), o agregÄƒm
    $ck = $pdo->prepare("
    ...
    if ($ex = $ck->fetch(PDO::FETCH_ASSOC)) {
      $upd = $pdo->prepare("
        UPDATE {$tabel_final_det_note}
         ...
         WHERE id_vanz=:id
      ");
      $upd->execute([':cant'=>$cant, ':tva'=>$tva, ':val'=>$val, ':val_cu'=>$val_cu, ':id'=>$ex['id_vanz']]);
      ...
      $ins = $pdo->prepare("
        INSERT INTO {$tabel_final_det_note}
          (nr_bon,cod_p,nume_produs,cantitate,cota_tva,tva_col,pret_vanzare,valoare_vanzare,valoare_vanzare_cu_tva,pachet,data,ora)
        VALUES
          (:nr,:cod,:nume,:cant,:cota,:tva,:pret,:val,:val_cu,:pachet,date('now','localtime'),time('now','localtime'))
      ");
      $ins->execute([
    ...
    // SGR â€“ identic cu produsul simplu (nu adÄƒugÄƒm pentru PRODUSE FINITE)
    if ($gestiune !== 'PRODUSE FINITE') {
      if ($sgr      === 1) adaugaGarantieSGR(-2, $cant, $nr_bon, $pachet, $pdo, $tabel_final_nomenclator, $tabel_final_det_note, $resp);
      if ($sgr_pet  === 1) adaugaGarantieSGR(-3, $cant, $nr_bon, $pachet, $pdo, $tabel_final_nomenclator, $tabel_final_det_note, $resp);
      if ($sgr_al   === 1) adaugaGarantieSGR(-4, $cant, $nr_bon, $pachet, $pdo, $tabel_final_nomenclator, $tabel_final_det_note, $resp);
      if ($sgr_st   === 1) adaugaGarantieSGR(-5, $cant, $nr_bon, $pachet, $pdo, $tabel_final_nomenclator, $tabel_final_det_note, $resp);
    }
  }
