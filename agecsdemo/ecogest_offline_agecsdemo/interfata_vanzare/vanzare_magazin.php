...
if (!$existing_bon) {
    $sql = "INSERT INTO $tabel_final_note
        (operator, locatie, cod_masa, status, cod_inchidere, nr_raport_z, cod_locatie,
         serie_casa_marcat, nui, serie_memorie_fiscala, valoare_vanzare_cu_tva, tva_colectata, discount, numerar, card, tichete, rest, protocol, glovo)
        VALUES
        (:adm_id, :locatie, :cod_masa, 'S', 0, 0, :locatie, :serie_casa_marcat, :nui, :serie_memorie_fiscala,
         0, 0, 0, 0, 0, 0, 0, 0, 0)";
...
<body>
    ...
    <div class="page-container">
        <header class="page-header">
            ...
            if ($client_agecs == 2) echo "<a href='vanzare_facturi.php'><button class='header-btn'>🧾 Facturi</button></a>";
            if ((int)$client_agecs === 8 && !empty($lorandSettings['operator_acces_rapoarte'])) {
                echo "<a href='rapoarte_produse_lorand.php' class='header-btn' style='text-decoration:none'>📊 Rapoarte + PROTOCOL</a>";
            }
            if ((int)$client_agecs === 8) echo "<a href='configurare_imprimanta_lorand.php' class='header-btn' style='text-decoration:none'>🖨️ Imprimantă</a>";
            ?>
...
<?php if (in_array($client_agecs, [17])): ?>
        ...
        <div class="input-group-append">
            <button class="btn btn-secondary" type="button" data-toggle="modal" data-target="#text-keyboard-modal" title="Deschide tastatura virtuală">
                <i class="fas fa-keyboard"></i>
                 ...
                 <?php if (!in_array((int)($_SESSION['client_id'] ?? 0), [17, 8], true)): ?>
<div class="form-group">
...
<input type="text"
  ...
  <!-- Buton Web Serial COM1 (ascuns implicit; va fi afișat prin JS doar dacă navigator.serial există) -->
        <?php if (in_array((int)($_SESSION['client_id'] ?? 0), [18]) && (int)$cod_locatie === 1): ?>
...
<?php endif; ?>
    ...
    <div class="grup-stanga">
        <form id="plata_form" method="POST" action="procesare_vanzare.php">
          ...
          <?php
    // Campul intern ramane glovo, dar pentru AGECSDEMO metoda este afisata ca ONLINE.
    if (in_array((int)$client_agecs, [17, 8], true)): ?>
...
const isClient1005 = ['1005', '1006'].includes(String(clientId));
...
// end micotex cantar
    ...
    const barcodeEnabled = String(clientId) !== '8' && barcodeFilterInput.length > 0;
    ...
    // Pentru AGECSDEMO 8 secțiunea de cod bare nu există și listener-ele nu sunt inițializate.
    if (barcodeEnabled) {
...
<?php if (in_array((int)($_SESSION['client_id'] ?? 0), [17])) { include('meniu_vanzare_widget.php'); } ?>
