...
if (!defined('AGECSDEMO_SQLITE_SCHEMA_VERSION')) {
    define('AGECSDEMO_SQLITE_SCHEMA_VERSION', 1);
}
...
function lorand_sqlite_apply_schema(PDO $pdo): void
{
    ...
    try {
        ...
        error_log('Nu s-a putut crea indexul fiscal AGECSDEMO pentru rapoarte_z: ' . $e->getMessage());
    }
    try {
        ...
        error_log('Nu s-a putut crea indexul AGECSDEMO pentru inchideri: ' . $e->getMessage());
    }
...
function lorand_sqlite_apply_schema_if_needed(PDO $pdo): bool
{
    ...
    if ($currentVersion >= AGECSDEMO_SQLITE_SCHEMA_VERSION) {
        if ($currentVersion > AGECSDEMO_SQLITE_SCHEMA_VERSION) {
            error_log(
                'Schema SQLite AGECSDEMO este mai noua decat aplicatia: baza=' . $currentVersion
                . ', aplicatie=' . AGECSDEMO_SQLITE_SCHEMA_VERSION
            );
        }
    ...
    try {
        ...
        $pdo->exec('PRAGMA user_version = ' . AGECSDEMO_SQLITE_SCHEMA_VERSION);
        $pdo->exec('COMMIT');
        $startedTransaction = false;
        $checkedConnections->attach($pdo);
        return true;
