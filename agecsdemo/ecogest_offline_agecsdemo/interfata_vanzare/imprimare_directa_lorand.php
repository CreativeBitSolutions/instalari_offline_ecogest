...
if ((int)offline_config_value('client_id', 0) !== 8
    || (int)($_SESSION['client_id'] ?? 0) !== 8
    || (int)($_SESSION['admin_id'] ?? 0) <= 0) {
    lorand_direct_print_response('error', 'access_denied', 'Sesiunea operatorului AGECSDEMO nu este activă.');
}
...
$claim = lorand_direct_print_claim(8, $locationId);
if ($claim === null) {
...
foreach ($documents as $index => $document) {
    ...
    try {
        ...
        $result = lorand_direct_print_windows_raw(
            $printerName,
            $plainText,
            'ECOGEST AGECSDEMO' . ($noteNumber > 0 ? ' bon ' . $noteNumber : '')
        );
        $printed++;
