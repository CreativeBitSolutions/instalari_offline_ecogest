...
function productReportValidateManager(PDO $pdo): void
{
    ...
    if ($clientId !== 8 || $operatorId <= 0 || empty($settings['operator_acces_rapoarte'])) {
        productReportResponse([
            'status' => 'error',
            'message' => 'Raportul este disponibil numai operatorilor autentificați din AGECSDEMO.',
        ], 403);
    }
}
...
try {
    ...
    $printerHelper = __DIR__ . '/printer_format_helper.php';
    ...
    error_log('rapoarte_produse_lorand_api: ' . $error->getMessage());
    productReportResponse([
        'status' => 'error',
