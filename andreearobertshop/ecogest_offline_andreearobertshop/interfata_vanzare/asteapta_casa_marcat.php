...
if ($clientId !== 22 || $locationId <= 0) {
    http_response_code(403);
    header('Content-Type: text/plain; charset=utf-8');
    echo 'Fluxul fiscal este disponibil numai pentru instalarea Andreea Robert Shop.';
    exit;
}

$queuePath = andreearobert_fiscal_queue_path($clientId, $locationId);
$archiveDirectory = dirname($queuePath) . DIRECTORY_SEPARATOR . 'bonuri_neprocesate';
$waitKey = $clientId . '_' . $locationId;
$maximumWait = 300;

function andreearobert_fiscal_signature(string $path): string
{
...
function andreearobert_fiscal_clear_wait(string $key): void
{
    unset($_SESSION['andreearobert_fiscal_wait'][$key]);
}

function andreearobert_fiscal_queue_helper_path(): string
{
    return andreearobert_offline_api_root()
        . DIRECTORY_SEPARATOR . 'printer_queue_atomic_helper.php';
}

function andreearobert_fiscal_archive_current(
    string $path,
    string $archive,
    string $expectedSignature
): ?bool {
    $queueHelper = andreearobert_fiscal_queue_helper_path();
    ...
    $claim = agecs_printer_queue_claim($path);
    ...
    if ($claimedPath === '' || andreearobert_fiscal_signature($claimedPath) !== $expectedSignature) {
        agecs_printer_queue_restore_claim($claimedPath, $path);
        return false;
    }

    if (!is_dir($archive) && !@mkdir($archive, 0777, true) && !is_dir($archive)) {
        agecs_printer_queue_restore_claim($claimedPath, $path);
        return false;
    }

    $archivePath = $archive . DIRECTORY_SEPARATOR
        . 'bon_neprocesat_' . date('Ymd_His') . '_'
        . substr($expectedSignature, 0, 10) . '.json';
...
function andreearobert_fiscal_status(
    string $path,
    string $archive,
    string $key,
    int $maximumWait
): array {
    $signature = andreearobert_fiscal_signature($path);
    if ($signature === '') {
        andreearobert_fiscal_clear_wait($key);
        return ['pending' => false, 'archived' => false, 'remaining' => 0, 'elapsed' => 0];
    }

    $state = $_SESSION['andreearobert_fiscal_wait'][$key] ?? [];
    if (($state['signature'] ?? '') !== $signature) {
        $state = ['signature' => $signature, 'started' => time()];
        $_SESSION['andreearobert_fiscal_wait'][$key] = $state;
    }
    ...
    $currentSignature = andreearobert_fiscal_signature($path);
    if ($currentSignature === '') {
        andreearobert_fiscal_clear_wait($key);
        return ['pending' => false, 'archived' => false, 'remaining' => 0, 'elapsed' => $elapsed];
    }
    if ($currentSignature !== $signature) {
        $_SESSION['andreearobert_fiscal_wait'][$key] = [
    ...
    try {
        $archived = andreearobert_fiscal_archive_current($path, $archive, $signature);
    ...
    return [
        ...
        'remaining' => 0,
        'elapsed' => $elapsed,
        'error' => 'Fișierul fiscal este încă blocat. Verificați scannerul casei de marcat.',
    ];
}

function andreearobert_fiscal_skip_pending(
    string $queuePath,
    ...
    if (($claim['status'] ?? '') === 'empty') {
        andreearobert_fiscal_clear_wait($waitKey);
        return [
            'code' => 'already_processed',
            'message' => 'Coada fusese deja preluată. Se continuă fără o nouă trimitere.',
        ];
    }
    if (($claim['status'] ?? '') !== 'claimed') {
    ...
    if (!is_dir($archiveDirectory)
        ...
        throw new RuntimeException(
            'Bonul oprit nu a putut fi arhivat. Coada normală a fost restaurată.'
        );
    }

    $archivePath = $archiveDirectory . DIRECTORY_SEPARATOR
        . 'bon_omis_manual_' . date('Ymd_His') . '_'
        . substr(hash('sha256', $claimedPath), 0, 10) . '.json';
    if (!@rename($claimedPath, $archivePath)) {
...
if (isset($_GET['status'])) {
    ...
    echo json_encode(
        andreearobert_fiscal_status($queuePath, $archiveDirectory, $waitKey, $maximumWait),
        JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES
    );
    exit;
}

$afterUrl = trim((string)($_SESSION['andreearobert_fiscal_after_url'] ?? ''));
$generator = 'casa_marcat_vanzare.php';
$directFiscalEnabled = true;
if (empty($_SESSION['andreearobert_direct_fiscal_csrf'])) {
    try {
        $_SESSION['andreearobert_direct_fiscal_csrf'] = bin2hex(random_bytes(24));
    } catch (Throwable $error) {
        $_SESSION['andreearobert_direct_fiscal_csrf'] =
            hash('sha256', session_id() . microtime(true));
    }
}
$directFiscalToken = (string)($_SESSION['andreearobert_direct_fiscal_csrf'] ?? '');
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST'
    ...
    if (!in_array($remoteAddress, ['127.0.0.1', '::1', '::ffff:127.0.0.1'], true)) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Operația este disponibilă numai în instalarea locală Andreea Robert Shop.',
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }

    $receivedToken = (string)($_POST['csrf_token'] ?? '');
    if ($directFiscalToken === '' || $receivedToken === ''
        || !hash_equals($directFiscalToken, $receivedToken)) {
        http_response_code(403);
        echo json_encode([
            'success' => false,
            'message' => 'Cererea a expirat. Reîncărcați pagina și încercați din nou.',
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }

    try {
        $result = andreearobert_fiscal_skip_pending($queuePath, $archiveDirectory, $waitKey);
        $nextUrl = $afterUrl !== '' ? $afterUrl : $generator;
        unset($_SESSION['andreearobert_fiscal_after_url']);
    ...
    <style>
        *{box-sizing:border-box}
        ...
        @media(max-width:650px){
            ...
            .direct-fiscal,.skip-fiscal{width:100%}
        }
    </style>
</head>
<body>
...
(function(){
    ...
    function complete(message,delay){
        ...
        title.textContent='Bonul a fost preluat pentru procesare';
