<?php
// nir_cu_discount_aplica_sporire.php
...
// Funcția de recalculare este identică și poate fi refolosită
...
$sporire_total = isset($_POST['sporire_value']) ? (float)$_POST['sporire_value'] : 0;
$update_preturi_catalog = isset($_POST['update_preturi_catalog']) && $_POST['update_preturi_catalog'] == '1';

if (empty($nr_nir) || $sporire_total <= 0) {
    echo json_encode(['success' => false, 'message' => 'Valoarea costului de transport este invalidă sau NIR-ul nu este identificat.']);
    exit;
}
...
try {
    ...
    // Calculează totalul de achiziție doar pentru produsele care NU sunt din gestiuni SGR.
    $sql_total = "
    ...
    if ($total_achizitie == 0) {
        throw new Exception("Nu există produse eligibile pe acest NIR pentru a adăuga costul de transport (produsele SGR sunt excluse).");
    }
    
    // Selectează pentru actualizare doar produsele care NU sunt din gestiuni SGR.
    $sql_produse = "
        SELECT a.*
        FROM achizitii a
        JOIN produse_servicii ps ON a.cod_p = ps.cod_produs
    ...
    foreach ($produse_achizitionate as $produs) {
        ...
        $sporire_produs = $sporire_total * $proportie;
        $sporire_unitar = $sporire_produs / $cantitate;
        
        // **MODIFICARE CHEIE:** Prețul se adună, nu se scade
        $pret_unitar_nou = (float)$produs['pret_unitar_achizitie'] + $sporire_unitar;
        ...
        // Recalcularea tuturor câmpurilor pe baza noului preț de achiziție
        ...
        // Actualizează prețul de achiziție în catalogul de produse dacă utilizatorul a fost de acord
    ...
    // **MODIFICARE CHEIE:** Se actualizează noua coloană 'sporire_pret_transp'
    $sql_update_nir_sporire = "UPDATE nir SET sporire_pret_transp = IFNULL(sporire_pret_transp, 0) + :sporire_total WHERE nr_nir = :nr_nir";
    $stmt_update_nir_sporire = $pdo->prepare($sql_update_nir_sporire);
    $stmt_update_nir_sporire->execute(['sporire_total' => $sporire_total, 'nr_nir' => $nr_nir]);

    // La final, se recalculează totalurile NIR-ului pe baza noilor valori din achizitii.
    actualizeaza_nir($pdo, $nr_nir);

    $pdo->commit();
    echo json_encode(['success' => true, 'message' => 'Costul de transport a fost adăugat cu succes!']);
    ...
    echo json_encode(['success' => false, 'message' => 'Eroare la adăugarea costului: ' . $e->getMessage()]);
}
?>