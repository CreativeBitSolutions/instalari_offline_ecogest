...
function andreearobert_fiscal_direct_response(
    bool $success,
    string $code,
    string $message,
    array $extra = []
...
function andreearobert_fiscal_direct_is_local(): bool
{
    $address = strtolower(trim((string)($_SERVER['REMOTE_ADDR'] ?? '')));
    return in_array($address, ['127.0.0.1', '::1', '::ffff:127.0.0.1'], true);
}

function andreearobert_fiscal_direct_log(string $message, array $context = []): void
{
    try {
        $directory = andreearobert_offline_api_root()
...
function andreearobert_fiscal_direct_load_config(string $path): array
{
    if (!is_file($path)) {
    ...
    if (!is_array($config)) {
        throw new RuntimeException(
            'Configurarea scannerului casei de marcat nu este un JSON valid.'
        );
    }
    return $config;
}

function andreearobert_fiscal_direct_normalize_content($content): string
{
    ...
    if (!preg_match('/^(?:K|H|S),1,/', $content)) {
        throw new RuntimeException(
            'Conținutul fiscal nu începe cu o comandă K, H sau S validă.'
        );
    }
    if (!preg_match('/(?:^|\r\n)S,1,/', $content)
        || !preg_match('/(?:^|\r\n)T,1,/', $content)) {
        throw new RuntimeException(
            'Conținutul fiscal nu conține liniile obligatorii S și T.'
        );
    }
    if (substr($content, -2) !== "\r\n") {
        $content .= "\r\n";
...
function andreearobert_fiscal_direct_extension(array $config): string
{
...
function andreearobert_fiscal_direct_atomic_write(
    string $directory,
    string $baseName,
    string $extension,
    string $content
    ...
    $temporaryPath = $finalPath . '.tmp.' . (string)getmypid() . '.'
    ...
    try {
        ...
        if (!@rename($temporaryPath, $finalPath)) {
            throw new RuntimeException(
                'Fișierul fiscal nu a putut fi publicat în folderul configurat.'
            );
        }
    } finally {
...
if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
    andreearobert_fiscal_direct_response(
        false,
        'method_not_allowed',
        'Metoda alternativă acceptă numai cereri locale POST.'
    );
}
if (!andreearobert_fiscal_direct_is_local()) {
    andreearobert_fiscal_direct_response(
        false,
        'local_only',
        'Trimiterea alternativă este disponibilă numai din aplicația locală.'
    );
}

$clientId = (int)($_SESSION['client_id'] ?? 0);
$locationId = (int)($_SESSION['cod_locatie'] ?? 0);
if ($clientId !== 22 || $locationId <= 0 || (int)($_SESSION['admin_id'] ?? 0) <= 0) {
    andreearobert_fiscal_direct_response(
        false,
        'forbidden',
        'Sesiunea operatorului Andreea Robert Shop nu este activă.'
    );
}

$expectedToken = (string)($_SESSION['andreearobert_direct_fiscal_csrf'] ?? '');
...
if ($expectedToken === '' || $receivedToken === ''
    ...
    andreearobert_fiscal_direct_response(
        false,
        'invalid_request',
        'Cererea a expirat. Reîncărcați pagina de așteptare.'
    );
}

try {
    $apiRoot = andreearobert_offline_api_root();
    ...
    if (!is_file($queueHelper)) {
        throw new RuntimeException(
            'Lipsește componenta care protejează coada casei de marcat.'
        );
    }
    require_once $queueHelper;

    $config = andreearobert_fiscal_direct_load_config($configPath);
    if ((int)($config['client_id'] ?? 0) !== $clientId) {
        throw new RuntimeException(
            'Clientul din configurarea scannerului nu corespunde sesiunii curente.'
        );
    }
    if ((int)($config['LocationId'] ?? 0) !== $locationId) {
        throw new RuntimeException(
            'Locația din configurarea scannerului nu corespunde sesiunii curente.'
        );
    }

    $destinationDirectory = trim((string)($config['CasaMarcatFolder'] ?? ''));
    $backupDirectory = trim((string)($config['BackupFolder'] ?? ''));
    $extension = andreearobert_fiscal_direct_extension($config);
    if ($destinationDirectory === '') {
        throw new RuntimeException('CasaMarcatFolder nu este configurat în scanner.');
    }
} catch (Throwable $error) {
    andreearobert_fiscal_direct_log(
        'Configurarea scannerului fiscal nu a putut fi folosită.',
        [
...
try {
    ...
    andreearobert_fiscal_direct_log(
        'Coada fiscală nu a putut fi revendicată.',
        ['error' => $error->getMessage()]
    );
    andreearobert_fiscal_direct_response(
        false,
        'claim_error',
        'Coada casei de marcat nu a putut fi accesată în siguranță.'
    );
}
...
if (($claim['status'] ?? '') === 'empty') {
    andreearobert_fiscal_direct_response(
        true,
        'already_claimed',
        'Bonul a fost deja preluat de scannerul casei de marcat.'
    );
}
if (($claim['status'] ?? '') !== 'claimed') {
    andreearobert_fiscal_direct_response(
        false,
        'claim_error',
        'Bonul nu a putut fi rezervat pentru trimiterea alternativă.'
    );
}
...
if ($documents === []) {
    ...
    andreearobert_fiscal_direct_response(
        true,
        'empty',
        'Coada fiscală nu conține bonuri.'
    );
}
...
try {
    foreach ($documents as $document) {
        ...
        $prepared[] = [
            'nrbon' => $nrBon,
            'content' => andreearobert_fiscal_direct_normalize_content(
                $document['continut_bon'] ?? ''
            ),
            'source' => $document,
        ];
    }
    ...
    $restored = agecs_printer_queue_restore_claim($claimedPath, $queuePath);
    andreearobert_fiscal_direct_log(
        'Conținutul fiscal nu a trecut validarea.',
        ['error' => $error->getMessage()]
    );
    andreearobert_fiscal_direct_response(
        false,
...
try {
    foreach ($prepared as $item) {
        $baseName = 'Bon_' . $item['nrbon'] . '_' . date('Ymd_His') . '_'
            . agecs_printer_queue_token();
        $finalPath = andreearobert_fiscal_direct_atomic_write(
        ...
        if ($backupDirectory !== ''
            ...
            try {
                andreearobert_fiscal_direct_atomic_write(
                    $backupDirectory,
                ...
                andreearobert_fiscal_direct_log(
                    'Fișierul principal a fost trimis, dar backupul a eșuat.',
                    [
                        'nrbon' => $item['nrbon'],
        ...
        andreearobert_fiscal_direct_log(
            'Fișier INP publicat direct, fără BOM.',
            [
                'nrbon' => $item['nrbon'],
                'file' => $finalPath,
            ]
    ...
    try {
        ...
        andreearobert_fiscal_direct_log(
            'Bonurile rămase nu au putut fi restaurate automat.',
            [
...
andreearobert_fiscal_direct_response(
    true,
    $removed ? 'written' : 'written_cleanup_warning',
    $message,
    [
        'written' => $writtenCount,
        'files' => $writtenFiles,
