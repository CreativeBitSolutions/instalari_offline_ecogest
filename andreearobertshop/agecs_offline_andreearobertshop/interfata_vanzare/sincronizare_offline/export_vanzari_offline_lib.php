...
function offline_export_build(PDO $pdo, $dataStartRaw, $dataEndRaw, $formatRaw = 'xml')
{
    ...
    $clientId = (int)offline_export_app_config_value('client_id', 22);
    ...
    if ($format === 'sql') {
        $output .= "-- Export offline vanzari Andreearobertshop\n";
    ...
    $ext = $format;
    $filename = "export_offline_andreearobertshop_{$data_start}_{$data_end}." . $ext;
