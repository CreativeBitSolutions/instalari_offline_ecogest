...
function agecs_printer_queue_lock(string $queuePath)
{
    ...
    if ($handle === false || !@flock($handle, LOCK_EX)) {
        ...
        throw new RuntimeException('Coada casei de marcat nu a putut fi blocată pentru actualizare.');
    }
    return $handle;
}
...
function agecs_printer_queue_append_documents(string $queuePath, array $documents, string $message): bool
{
    ...
    $temporaryPath = $queuePath . '.tmp.' . (string)getmypid() . '.' . agecs_printer_queue_token();
    $backupPath = '';
    ...
    try {
        ...
        if (is_file($queuePath)) {
            ...
            if (!is_array($decoded) || !isset($decoded['data']) || !is_array($decoded['data'])) {
                throw new RuntimeException('Documentul existent din coada casei de marcat nu este valid.');
            }
        ...
        if (!is_string($json)
            || @file_put_contents($temporaryPath, $json, LOCK_EX) !== strlen($json)) {
            throw new RuntimeException('Fișierul temporar al cozii nu a putut fi scris integral.');
        }
...
function agecs_printer_queue_claim(string $queuePath): array
{
    ...
    try {
        ...
        $claimedPath = $queuePath . '.processing.' . (string)getmypid() . '.' . agecs_printer_queue_token();
...
function agecs_printer_queue_restore_claim(string $claimedPath, string $queuePath): bool
{
    ...
    $lock = agecs_printer_queue_lock($queuePath);
    try {
        if (!is_file($queuePath) && @rename($claimedPath, $queuePath)) {
            return true;
        }
        return false;
    } finally {
