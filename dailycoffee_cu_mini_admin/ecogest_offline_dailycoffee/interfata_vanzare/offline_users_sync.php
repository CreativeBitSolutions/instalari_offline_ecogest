...
function ous_is_dailycoffee_offline_operator(array $user, int $clientId, int $codLocatie): bool
{
    if ($clientId !== 2 || $codLocatie !== 2) {
        return false;
    }

    $email = strtolower(trim((string)($user['admin_email_address'] ?? '')));
    if ($email === 'offline.c2.l2.o2@agecs.local') {
        return true;
    }

    return strtoupper(trim((string)($user['admin_firstname'] ?? ''))) === 'OPERATOR OFFLINE'
        && strtoupper(trim((string)($user['admin_lastname'] ?? ''))) === 'CLIENT 2 LOCATIA 2 ID 2';
}
...
try {
    ...
    if ($clientId === 2 && $codLocatie === 2) {
        $users = array_values(array_filter(
            $users,
            static fn($user): bool => is_array($user) && ous_is_dailycoffee_offline_operator($user, $clientId, $codLocatie)
        ));
        if (count($users) !== 1) {
            throw new RuntimeException('Raspunsul online nu contine operatorul offline dedicat pentru Daily Coffee. Datele locale au fost pastrate.');
        }
    }
