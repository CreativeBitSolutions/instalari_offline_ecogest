...
$sql = "
    SELECT 
        ...
        SUM(a.valoare_achizitie) AS total_valoare_fara_tva,
        SUM(a.valoare_tva_achizitie) AS total_tva,
        SUM(a.valoare_achizitie_cu_tva) AS total_valoare_factura,
        SUM(a.valoare_pret_amanunt) AS total_valoare_cu_adaos
    FROM nir n
...
class PDF_NIR_STD_P extends FPDF {
    public $firma;
    public $start_date;
    public $end_date;
    public $cota_tva;
    ...
    function Header() {
        if (!empty($this->firma)) {
            ...
            $this->Cell(0,7,remove_diacritics($this->firma['den_ent']),0,1,'C');
        }
        $this->SetFont('Arial','B',13);
        $this->Cell(0,7,'Raport Note de Receptie (NIR)',0,1,'C');
        $this->SetFont('Arial','',9);
        $this->Cell(0,5,'Perioada: '.date('d.m.Y',strtotime($this->start_date)).' - '.date('d.m.Y',strtotime($this->end_date)),0,1,'C');
        if ($this->cota_tva !== null) {
            $this->SetFont('Arial','B',9);
            $this->Cell(0,5,'Filtrat pentru Cota TVA: '.$this->cota_tva.'%',0,1,'C');
        }
        $this->Ln(3);
        ...
        for($i=0;$i<count($header);$i++){
            $this->Cell($this->widths[$i],5.2,$header[$i],1,0,'C');
        }
        $this->Ln();
    }
    ...
    function Footer() {
        ...
        $this->Cell(0,6,'Pagina '.$this->PageNo(),0,0,'C');
    }
    ...
    function Row($data) {
        ...
        for($i=0;$i<count($data);$i++){
            ...
            $this->Rect($x,$y,$w,$h);
            $this->MultiCell($w,4.2,$data[$i],0,$a);
            $this->SetXY($x+$w,$y);
        }
        $this->Ln($h);
    }
...
// latimi ajustate portret, aceleasi 11 coloane
$pdf->SetWidths([28,16,30,18,14,14,10,18,16,18,18]);
...
$filename_parts = ['Raport_NIR_Standard_Portrait','de_la_'.$start_date,'pana_la_'.$end_date];
if ($cota_tva !== null) $filename_parts[] = 'TVA_'.$cota_tva;
