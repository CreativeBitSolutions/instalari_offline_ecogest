...
if(isset($_POST['creaza_nir'])) {
    ...
    offline_sequence_record($pdo, 'nr_nir', (int)$nr_nir, (int)$cod_locatie, '', 'nir_created', 'nir', (string)$nr_nir);
    $createdNirStmt = $pdo->prepare('SELECT id_nir FROM nir WHERE nr_nir = ? LIMIT 1');
    $createdNirStmt->execute([$nr_nir]);
    $createdNirId = (int)$createdNirStmt->fetchColumn();
    if ($createdNirId > 0) {
        offline_sync_enqueue_nir_safely($pdo, $createdNirId);
    }
