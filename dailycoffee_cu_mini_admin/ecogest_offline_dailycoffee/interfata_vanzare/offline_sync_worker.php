...
try {
    ...
    $stmt = $pdo->query("SELECT * FROM offline_sync_outbox
        ...
        ORDER BY CASE WHEN aggregate_type = 'catalog' THEN 0 ELSE 1 END, id LIMIT 1");
    ...
    if ((string)($event['aggregate_type'] ?? '') === 'catalog' && !$config['catalog_push_enabled']) {
        $stmt = $pdo->prepare("UPDATE offline_sync_outbox
            SET status = 'sent', sent_at = CURRENT_TIMESTAMP, locked_at = NULL,
                last_error = 'Catalogul este administrat online.'
            WHERE id = ?");
        $stmt->execute([(int)$event['id']]);
        offline_sync_worker_release($pdo, $token);
        offline_sync_worker_json(200, [
            'status' => 'skipped',
            'message' => 'Catalogul Daily Coffee este administrat online.',
            'queue' => offline_sync_queue_counts($pdo),
        ]);
    }
