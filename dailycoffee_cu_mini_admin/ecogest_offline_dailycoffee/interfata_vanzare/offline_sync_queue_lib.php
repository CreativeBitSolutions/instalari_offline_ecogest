...
function offline_sync_queue_config(): array
{
    ...
    return [
        ...
        'catalog_push_enabled' => !array_key_exists('catalog_push_enabled', $config) || (bool)$config['catalog_push_enabled'],
...
function offline_sync_queue_pk(string $table): string
{
    $map = [
        ...
        'categorii' => 'id_categorie', 'categorii_locatii' => 'id', 'produse_servicii' => 'cod_produs',
    ];
    return $map[$table] ?? 'id';
}
...
function offline_sync_queue_xml(array $tables, int $location): string
{
    ...
    if ((int)$config['client_id'] === 2) {
        $order = array_merge(['categorii', 'categorii_locatii', 'produse_servicii'], $order);
    }
...
function offline_sync_queue_sale_payload(PDO $pdo, int $nrBon): ?array
{
    ...
    $reportSeries = trim((string)($note['serie_casa_marcat'] ?? ''));
    if ($reportSeries === '0') {
        $reportSeries = '';
    }
    ...
    $tables['inchideri_r_12'] = $closure > 0
        ? offline_sync_queue_rows(
            $pdo,
            'inchideri_r_12',
            "cod_inchidere = ? AND CAST(COALESCE(NULLIF(cod_locatie, 0), locatie, 0) AS INTEGER) = ? AND (COALESCE(serie_casa_marcat, '') = ? OR COALESCE(serie_casa_marcat, '') = '')",
            [$closure, $location, $reportSeries],
            'id_inch'
        )
        : [];
    if ($report > 0 && $reportSeries === '') {
        ...
        if (count($candidateSeries) === 1) {
            $reportSeries = $candidateSeries[0];
        } elseif (count($candidateSeries) > 1) {
            $reportSeries = null;
        }
    }
...
function offline_sync_queue_nir_payload(PDO $pdo, int $idNir): ?array
{
    ...
    $nirRows = offline_sync_queue_rows($pdo, 'nir', 'id_nir = ?', [$idNir]);
    if (!$nirRows) {
        return null;
    }
    $nir = $nirRows[0];
    $nrNir = (int)($nir['nr_nir'] ?? 0);
    $location = (int)($nir['cod_locatie'] ?? $nir['locatie'] ?? offline_sync_queue_config()['cod_locatie']);
    if ($location <= 0) {
        $location = (int)offline_sync_queue_config()['cod_locatie'];
    }
    $tables = [
...
function offline_sync_queue_closure_payload(PDO $pdo, int $idInchidere): ?array
{
    $rows = offline_sync_queue_rows($pdo, 'inchideri_r_12', 'id_inch = ?', [$idInchidere], 'id_inch');
    if (!$rows) {
        return null;
    }
    ...
    $xml = offline_sync_queue_xml(['inchideri_r_12' => $rows], $location);
    return ['xml' => $xml, 'hash' => hash('sha256', $xml), 'location' => $location];
}

function offline_sync_queue_report_payload(PDO $pdo, int $idRaport): ?array
{
    $rows = offline_sync_queue_rows($pdo, 'rapoarte_z', 'id = ?', [$idRaport], 'id');
    if (!$rows) {
        return null;
    }
    $location = (int)($rows[0]['cod_locatie'] ?? $rows[0]['locatie'] ?? offline_sync_queue_config()['cod_locatie']);
    ...
    $xml = offline_sync_queue_xml(['rapoarte_z' => $rows], $location);
    return ['xml' => $xml, 'hash' => hash('sha256', $xml), 'location' => $location];
}
...
function offline_sync_enqueue_nir(PDO $pdo, int $idNir): bool
{
    ...
    return $payload ? offline_sync_queue_store($pdo, 'nir_state', 'nir', (string)$idNir, $payload) : false;
}
...
function offline_sync_queue_nir_deleted_payload(PDO $pdo, array $nir): ?array
{
    if ((int)offline_sync_queue_config()['client_id'] !== 2 || (int)($nir['id_nir'] ?? 0) <= 0) {
        return null;
    }
    $nir['offline_deleted'] = 1;
    $location = (int)($nir['cod_locatie'] ?? $nir['locatie'] ?? offline_sync_queue_config()['cod_locatie']);
    if ($location <= 0) {
        $location = (int)offline_sync_queue_config()['cod_locatie'];
    }
    $xml = offline_sync_queue_xml(['nir' => [$nir]], $location);
    return ['xml' => $xml, 'hash' => hash('sha256', $xml), 'location' => $location];
}

function offline_sync_enqueue_nir_deleted_safely(PDO $pdo, array $nir): bool
{
    try {
        $payload = offline_sync_queue_nir_deleted_payload($pdo, $nir);
        $idNir = (string)(int)($nir['id_nir'] ?? 0);
        return $payload && $idNir !== '0'
            ? offline_sync_queue_store($pdo, 'nir_deleted', 'nir', $idNir, $payload)
            : false;
    } catch (Throwable $e) {
        error_log('Nu s-a putut inscrie stergerea NIR-ului in coada offline: ' . $e->getMessage());
        return false;
    }
}

function offline_sync_enqueue_closure_safely(PDO $pdo, int $idInchidere): bool
{
    try {
        $payload = offline_sync_queue_closure_payload($pdo, $idInchidere);
        return $payload ? offline_sync_queue_store($pdo, 'closure_state', 'closure', (string)$idInchidere, $payload) : false;
    } catch (Throwable $e) {
        error_log('Nu s-a putut inscrie inchiderea ' . $idInchidere . ' in coada offline: ' . $e->getMessage());
        return false;
    }
}

function offline_sync_enqueue_report_safely(PDO $pdo, int $idRaport): bool
{
    try {
        $payload = offline_sync_queue_report_payload($pdo, $idRaport);
        return $payload ? offline_sync_queue_store($pdo, 'report_z_state', 'report_z', (string)$idRaport, $payload) : false;
    } catch (Throwable $e) {
        error_log('Nu s-a putut inscrie raportul Z ' . $idRaport . ' in coada offline: ' . $e->getMessage());
        return false;
    }
}

function offline_sync_queue_catalog_payload(PDO $pdo): ?array
{
    $config = offline_sync_queue_config();
    if ((int)$config['client_id'] !== 2 || (string)$config['profile'] !== 'dailycoffee' || !$config['catalog_push_enabled']) {
        return null;
    }

    $tables = [];
    foreach (['categorii', 'categorii_locatii', 'produse_servicii'] as $table) {
        $tables[$table] = offline_sync_queue_rows($pdo, $table, '1=1', [], offline_sync_queue_pk($table));
    }
    if (!$tables['categorii'] && !$tables['categorii_locatii'] && !$tables['produse_servicii']) {
        return null;
    }

    $xml = offline_sync_queue_xml($tables, (int)$config['cod_locatie']);
    return ['xml' => $xml, 'hash' => hash('sha256', $xml), 'location' => (int)$config['cod_locatie']];
}

function offline_sync_enqueue_catalog_safely(PDO $pdo): bool
{
    try {
        $payload = offline_sync_queue_catalog_payload($pdo);
        return $payload ? offline_sync_queue_store($pdo, 'catalog_state', 'catalog', 'dailycoffee', $payload) : false;
    } catch (Throwable $e) {
        error_log('Nu s-a putut inscrie catalogul Daily Coffee in coada offline: ' . $e->getMessage());
        return false;
    }
}
...
function offline_sync_queue_discover(PDO $pdo, int $limit = 40): int
{
    ...
    $queued = 0;
    if ((int)offline_sync_queue_config()['client_id'] === 2) {
        $catalogPayload = offline_sync_queue_catalog_payload($pdo);
        if ($catalogPayload) {
            $state = $pdo->prepare("SELECT payload_sha256 FROM offline_sync_entity_state WHERE entity_type = 'catalog' AND entity_id = 'dailycoffee'");
            $state->execute();
            if ((string)$state->fetchColumn() !== $catalogPayload['hash']) {
                $queued += offline_sync_enqueue_catalog_safely($pdo) ? 1 : 0;
            }
        }
    }
    if (offline_sync_queue_table_exists($pdo, 'inchideri_r_12')) {
        foreach (offline_sync_queue_rows($pdo, 'inchideri_r_12', 'CAST(COALESCE(cod_inchidere, 0) AS INTEGER) > 0', [], 'id_inch') as $closure) {
            $idInchidere = (int)($closure['id_inch'] ?? 0);
            if ($idInchidere <= 0) {
                continue;
            }
            $payload = offline_sync_queue_closure_payload($pdo, $idInchidere);
            ...
            $state = $pdo->prepare("SELECT payload_sha256 FROM offline_sync_entity_state WHERE entity_type = 'closure' AND entity_id = ?");
            $state->execute([(string)$idInchidere]);
            if ((string)$state->fetchColumn() !== $payload['hash']) {
                $queued += offline_sync_queue_store($pdo, 'closure_state', 'closure', (string)$idInchidere, $payload) ? 1 : 0;
            }
        }
    }
    if (offline_sync_queue_table_exists($pdo, 'rapoarte_z')) {
        foreach (offline_sync_queue_rows($pdo, 'rapoarte_z', 'CAST(COALESCE(nr_raport_z, 0) AS INTEGER) > 0', [], 'id') as $report) {
            $idRaport = (int)($report['id'] ?? 0);
            if ($idRaport <= 0) {
                continue;
            }
            $payload = offline_sync_queue_report_payload($pdo, $idRaport);
            ...
            $state = $pdo->prepare("SELECT payload_sha256 FROM offline_sync_entity_state WHERE entity_type = 'report_z' AND entity_id = ?");
            $state->execute([(string)$idRaport]);
            if ((string)$state->fetchColumn() !== $payload['hash']) {
                $queued += offline_sync_queue_store($pdo, 'report_z_state', 'report_z', (string)$idRaport, $payload) ? 1 : 0;
            }
        }
    }
    ...
    if ((int)offline_sync_queue_config()['client_id'] === 2 && offline_sync_queue_table_exists($pdo, 'nir')) {
        $stmt = $pdo->prepare("SELECT id_nir FROM nir n
            WHERE NOT EXISTS (SELECT 1 FROM offline_sync_entity_state s WHERE s.entity_type = 'nir' AND s.entity_id = CAST(n.id_nir AS TEXT))
            ORDER BY id_nir LIMIT ?");
        $stmt->bindValue(1, $limit, PDO::PARAM_INT);
        $stmt->execute();
        $nirIds = $stmt->fetchAll(PDO::FETCH_COLUMN);
        $stmt = $pdo->prepare("SELECT id_nir FROM nir ORDER BY id_nir DESC LIMIT ?");
        $stmt->bindValue(1, $limit, PDO::PARAM_INT);
        $stmt->execute();
        ...
        foreach ($nirIds as $idNir) {
            ...
            if ((string)$state->fetchColumn() !== $payload['hash']) {
                $queued += offline_sync_queue_store($pdo, 'nir_state', 'nir', (string)$idNir, $payload) ? 1 : 0;
            }
        }
    }
