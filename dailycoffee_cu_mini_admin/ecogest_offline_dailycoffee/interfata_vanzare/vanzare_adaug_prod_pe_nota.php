...
try {
    ...
    if ($isBarcodeLookup) {
        ...
        if (!$produs_info) {
            ...
            $stmt = $pdo->prepare("SELECT cod_produs FROM produse_servicii n
                ...
                LIMIT 1");
            $stmt->execute([':cod_bare' => $barcodeRaw, ':cod_locatie' => $cod_locatie]);
            ...
            $stmt_produs->execute([':cod_p' => $cod_p, ':cod_locatie' => $cod_locatie]);
    ...
    $availability = $pdo->prepare("SELECT 1 FROM produse_servicii n
        ...
        LIMIT 1");
    $availability->execute([':cod_produs' => (int)$cod_p, ':cod_locatie' => $cod_locatie]);
