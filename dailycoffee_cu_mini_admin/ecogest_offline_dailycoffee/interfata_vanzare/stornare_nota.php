...
<?php 
 ...
 $f_tot_sql = "SELECT sum($tabel_final_det_note.valoare_vanzare) as a from $tabel_final_det_note  where nr_bon='$nr_bon'; ";    
...
while ($row = $f_tot_stmt->fetch(PDO::FETCH_ASSOC)){
 ...
 $f_tot_sql = "SELECT sum($tabel_final_det_note.tva_col) as b from $tabel_final_det_note  where nr_bon='$nr_bon'; ";    
$f_tot_stmt = $pdo->prepare($f_tot_sql);  
$f_tot_stmt->execute();
...
while ($row = $f_tot_stmt->fetch(PDO::FETCH_ASSOC)){
 ...
 $f_tot_sql = "SELECT sum($tabel_final_det_note.valoare_vanzare_cu_tva) as c from $tabel_final_det_note where nr_bon='$nr_bon'; ";    
$f_tot_stmt = $pdo->prepare($f_tot_sql);  
$f_tot_stmt->execute();
...
$f_sql = "SELECT $tabel_final_det_stornari_rest.id_vanzare,$tabel_final_det_stornari_rest.cod_p,$tabel_final_nomenclator.den_p,$tabel_final_nomenclator.um,$tabel_final_det_stornari_rest.cantitate,$tabel_final_det_stornari_rest.tva_col,$tabel_final_det_stornari_rest.pret_vanzare,$tabel_final_det_stornari_rest.valoare_vanzare,$tabel_final_det_stornari_rest.valoare_vanzare_cu_tva,$tabel_final_det_stornari_rest.id from $tabel_final_nomenclator inner join $tabel_final_det_stornari_rest on $tabel_final_det_stornari_rest.cod_p=$tabel_final_nomenclator.cod_p where id_stornare='$id_storn';";    
$f_stmt = $pdo->prepare($f_sql);  
$f_stmt->execute(); 
...
while ($row = $f_stmt->fetch(PDO::FETCH_ASSOC)){ 
    ...
    $df_sql = "SELECT cantitate from $tabel_final_det_note where nr_bon='$nr_bon' AND cod_p='$cd_p';";    
...
echo"<td>".$valoare_vanzare_c_tva."</td>";
 ...
 echo " </tr>";
  ...
  if (isset($_POST[$id_simplu_vanz])) {
					...
					$sterg_sql="SELECT * FROM $tabel_final_det_stornari_rest where $tabel_final_det_stornari_rest.id = '$id' ";
...
$cant_vand=$row['cantitate'];
					...
					$sql="UPDATE $tabel_final_det_stornari_rest set tva_col=0,valoare_vanzare_cu_tva=0,discount=0,pret_vanzare=0,valoare_vanzare=0,cantitate=0 WHERE $tabel_final_det_stornari_rest.id = '$id';";
...
<?php 
 ...
 $f_tot_sql = "SELECT sum($tabel_final_det_stornari_rest.valoare_vanzare) as d from $tabel_final_det_stornari_rest  where id_stornare='$id_storn'; ";    
$f_tot_stmt = $pdo->prepare($f_tot_sql);  
$f_tot_stmt->execute();
...
while ($row = $f_tot_stmt->fetch(PDO::FETCH_ASSOC)){
 ...
 $f_tot_sql = "SELECT sum($tabel_final_det_stornari_rest.tva_col) as e from $tabel_final_det_stornari_rest  where id_stornare='$id_storn'; ";    
$f_tot_stmt = $pdo->prepare($f_tot_sql);  
$f_tot_stmt->execute();
...
while ($row = $f_tot_stmt->fetch(PDO::FETCH_ASSOC)){
 ...
 $f_tot_sql = "SELECT sum($tabel_final_det_stornari_rest.valoare_vanzare_cu_tva) as f from $tabel_final_det_stornari_rest where id_stornare='$id_storn'; ";    
$f_tot_stmt = $pdo->prepare($f_tot_sql);  
$f_tot_stmt->execute();
...
//$test_sql = "SELECT $tabel_final_det_note.cod_p,$tabel_final_nomenclator.den_p,$tabel_final_nomenclator.um,$tabel_final_det_note.cantitate as cant_bon,sum($tabel_final_det_stornari_rest.cantitate),$tabel_final_det_note.tva_col,$tabel_final_det_note.pret_vanzare,$tabel_final_det_note.valoare_vanzare,$tabel_final_det_note.valoare_vanzare_cu_tva,$tabel_final_det_note.id_vanz from $tabel_final_nomenclator inner join $tabel_final_det_note on $tabel_final_det_note.cod_p=$tabel_final_nomenclator.cod_p inner join $tabel_final_det_stornari_rest on $tabel_final_det_stornari_rest.id_vanzare=$tabel_final_det_note.id_vanz where $tabel_final_det_note.nr_bon='$nr_bon' and $tabel_final_det_note.cantitate>sum($tabel_final_det_stornari_rest.cantitate) ;";    
$test_sql = "SELECT $tabel_final_det_stornari_rest.id_vanzare,$tabel_final_det_note.cantitate as cant_bon,sum($tabel_final_det_stornari_rest.cantitate) as cantitate_total_stornata,$tabel_final_nomenclator.den_p,$tabel_final_det_note.id_vanz from $tabel_final_nomenclator inner join $tabel_final_det_note on $tabel_final_det_note.cod_p=$tabel_final_nomenclator.cod_p inner join $tabel_final_det_stornari_rest on $tabel_final_det_stornari_rest.id_vanzare=$tabel_final_det_note.id_vanz where $tabel_final_det_note.nr_bon='$nr_bon' group by $tabel_final_det_stornari_rest.id_vanzare";
...
' class='btn btn-primary btn-block' type='submit' name='$id_vanz'>$den_p</button></form></td>
 ...
 $produs=$cod_p;
	 $psql2 = "SELECT * from $tabel_final_det_note where id_vanz='$id_vanz';";    
...
$cantitate=$row['cantitate'];
    ...
    $psql = "update $tabel_final_det_stornari_rest set cantitate=cantitate+'$cantitate',pret_vanzare='$pret_vanzare',tva_col=tva_col+'$tva_col',valoare_vanzare=valoare_vanzare+'$valoare_vanzare',valoare_vanzare_cu_tva=valoare_vanzare_cu_tva+'$valoare_vanzare_cu_tva',discount=discount+'$discount' where id_stornare='$id_storn' and id_vanzare='$id_vanz'"; 
...
<form method='POST'> <input type='submit' class='btn btn-primary btn-block' name='stornare_nota' value='Stornare'/></form>
...
if(isset($_POST['anuleaza_storn'])){
           	...
           	$sql="DELETE from $tabel_final_stornari_rest WHERE $tabel_final_stornari_rest.id_stornare ='$id_storn';
					DELETE FROM $tabel_final_det_stornari_rest WHERE $tabel_final_det_stornari_rest.id_stornare ='$id_storn';";
$pdo->exec($sql) or die(print_r($pdo->errorInfo(), true));
	unset($_SESSION['nr_bon']);

			printf("<script>location.href='lista_note.php'</script>");
           
       }
if(isset($_POST['stornare_nota'])){
...
$fin_sql = "update $tabel_final_stornari_rest set nr_bon='$nr_bon',operator='$adm_id',tva_colectata='$storn_total_tva_col',valoare_fara_tva='$storn_valoare_f_vz',data_stornare='$data_bon',ora_stornare='$ora_bon',status='F' where id_stornare='$id_storn'"; 
	try{
...
$misc_sql = "SELECT $tabel_final_det_stornari_rest.cod_p,$tabel_final_det_stornari_rest.pret_vanzare,$tabel_final_det_stornari_rest.cantitate,$tabel_final_nomenclator.gestiune from $tabel_final_det_stornari_rest INNER JOIN $tabel_final_nomenclator on $tabel_final_det_stornari_rest.cod_p=$tabel_final_nomenclator.cod_p where $tabel_final_det_stornari_rest.nr_bon='$nr_bon' and $tabel_final_det_stornari_rest.id_stornare='$id_storn' and $tabel_final_det_stornari_rest.cantitate>0;";     
...
$stocstmt->execute(); 
    $iessql = "insert into $tabel_final_miscari(data,cod_p,cantitate_misc,tip_miscare,fel_doc,nr_doc,pret_vanzare) values('$data_bon','$prod','$qt','I','BS','$id_storn','$pv');";   
...
<?php if(isset($_POST['modific_cant'])){
    ...
    $id_vz=$_POST['idvz'];
		         	     ...
		         	     $psql5 = "UPDATE $tabel_final_det_stornari_rest set tva_col=0,valoare_vanzare_cu_tva=0,discount=0,pret_vanzare=0,valoare_vanzare=0,cantitate=0 WHERE $tabel_final_det_stornari_rest.id = '$id_vz';"; 
...
$pdo->exec($psql5) or die(print_r($pdo->errorInfo(), true));   
    ...
    echo $psql5 . "<br>" . $e->getMessage();
 	 ...
 	 $psql2M = "SELECT * from $tabel_final_det_note where id_vanz='$id_vanzaree';";   
