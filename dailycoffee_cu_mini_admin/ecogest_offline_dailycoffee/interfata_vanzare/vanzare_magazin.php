...
if (!$existing_bon) {
    $next_bon_sql = "SELECT COALESCE(MAX(nrbon), 0) + 1 AS next_nrbon FROM $tabel_final_note";
    $next_bon = (int) $pdo->query($next_bon_sql)->fetchColumn();

    $sql = "INSERT INTO $tabel_final_note
                (nrbon, serie, data_bon, ora_bon,
    ...
    $stmt->execute([
        ':nr_bon'   => $next_bon,
        ':serie'    => '',          // sau 'MAG', 'A1', etc., cum vrei tu
        ':adm_id'   => $adm_id,
        ':locatie'  => $cod_locatie,
        ':cod_masa' => $cod_masa,
            ...
            if ($row) {
                ...
                if ($discount_unitar > 0 && $discount_unitar <= $pret_unitar_initial) {
                    ...
                    $sql_insert_discount = "INSERT INTO discounturi_acordate 
                                            (id_vanz, id_operator, cod_locatie, tip_discount, valoare_procent, valoare_discount_ron, pret_unitar_initial, pret_unitar_final) 
                                            VALUES (:id_vanz, :id_op, :cod_locatie, :tip, :proc, :val_ron, :pret_init, :pret_fin)";
...
<div class="modal fade" id="exportOfflineModal" tabindex="-1" role="dialog" aria-labelledby="exportOfflineLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="exportOfflineForm" method="post" action="export_vanzari_offline.php" target="_blank">
                <div class="modal-header">
                    <h5 class="modal-title" id="exportOfflineLabel">Export BD</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="exportDataStart">De la data</label>
                        <input type="date" class="form-control" id="exportDataStart" name="data_start" required>
                    </div>

                    <div class="form-group">
                        <label for="exportDataEnd">Până la data</label>
                        <input type="date" class="form-control" id="exportDataEnd" name="data_end" required>
                    </div>

                    <div class="form-group">
                        <label for="exportFormat">Format fișier</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary">Generează fișier</button>
                </div>
            </form>
...
<div class="modal fade" id="adminModal" tabindex="-1" role="dialog" aria-labelledby="adminModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="admin-link-grid">
                    <a href="note_receptie.php">
                        <i class="fas fa-truck-loading" style="color:#f59e0b;"></i> NIR
                        <small>Note de receptie</small>
                    </a>
                    <a href="creare_nir.php">
                        <i class="fas fa-file-circle-plus"></i> NIR manual
                        <small>Creare antet si receptie</small>
                    </a>
                    <a href="furnizori_admin.php">
                        <i class="fas fa-address-book"></i> Furnizori
                        <small>CRUD furnizori NIR</small>
                    </a>
                    <a href="#" class="disabled" aria-disabled="true" title="Gestionare în online" onclick="return false;" style="opacity:.65;cursor:not-allowed;">
                        <i class="fas fa-box"></i> Produse
                        <small>Gestionare în online</small>
                    </a>
                    <a href="#" class="disabled" aria-disabled="true" title="Gestionare în online" onclick="return false;" style="opacity:.65;cursor:not-allowed;">
                        <i class="fas fa-tags"></i> Categorii
                        <small>Gestionare în online</small>
                    </a>
                    <a href="config_firma.php">
                        <i class="fas fa-building"></i> Date firma
                        <small>Configurare firma</small>
                    </a>
                    <a href="vanzari_pos.php">
                        <i class="fas fa-receipt"></i> Vanzari POS
                        <small>Bonuri si detalii vanzare</small>
                    </a>
                </div>
            </div>
        </div>
    ...
    $bonuri_neinchise_tura = (int)$bon_stmt->fetchColumn();
    if ($bonuri_neinchise_tura >= 1) {
        ...
        echo "<button type='button' class='header-btn btn-warning' disabled title='Nu exista bonuri finalizate neinchise pentru operatorul curent'>🌇 Închide Tura</button>";
    }
    ?>
<?php endif; ?>
                ...
                <?php else: ?>
                    <button type="button" class="header-btn btn-danger" disabled title="Nu există ture închise fără raport Z">
                        <i class="fas fa-file-invoice"></i> Raport Z
                    </button>
                <?php endif; ?>
                 <div class="user-info">
...
<?php endif; ?>
                        ...
                        <?php
                            $categ_sql = "SELECT id_categorie, den_categ FROM $tabel_final_categorii WHERE se_vinde='1'";
                            if ((int)$client_agecs === 2) {
                                $categ_sql .= " AND EXISTS (
                                    SELECT 1
                                    FROM produse_servicii p_loc
                                    INNER JOIN produse_servicii_locatii psl_loc
                                        ON psl_loc.cod_produs = p_loc.cod_produs
                                    WHERE p_loc.id_categorie = $tabel_final_categorii.id_categorie
                                      AND p_loc.activ = 1
                                      AND psl_loc.cod_locatie = " . (int)$cod_locatie . "
                                      AND psl_loc.activ = 1
                                )";
                            }
                            $categ_sql .= " ORDER BY den_categ ASC;";
...
<?php endif; ?>
                    <div class="form-group">
                        ...
                        <div class="input-group">
<?php $initialQty = ($_SESSION['client_id'] == 2) ? '' : '1'; ?>
<input type="text"
...
<?php endif; ?>
<?php if ((int)($_SESSION['client_id'] ?? 0) === 24 && (int)$cod_locatie === 1): ?>
  <button id="citestecantarmicotexcom3"
          class="btn btn-warning"
          type="button"
          title="Citește cântar Micotex (COM3)"
          style="display:none;">
    <i class="fa fa-plug"></i>Cantar
  </button>
<?php elseif ((int)($_SESSION['client_id'] ?? 0) === 24 && (int)$cod_locatie === 2): ?>
...
<?php endif; ?>
    ...
    $sql_relist = "
        ...
        AND status = 'F' 
        AND datetime(data_bon || ' ' || ora_bon) >= datetime('now', '-24 hours')
        AND operator = :adm_id 
        AND locatie = :locatie 
        ORDER BY nrbon DESC
    ";
...
(function initMicotexCOM3() {
  ...
  if (!btn) return;
  if (!('serial' in navigator)) return; // necesită Chrome/Edge + HTTPS/localhost
  btn.style.display = 'inline-block';
  ...
  // Parser „stil FoxPro” pentru COM3:
  // v1 = SUBSTR(cc, LEN(cc)-ATCC(cc,'kg')-38, 6)
  // semn = SUBSTR(cc, LEN(cc)-ATCC(cc,'kg')-40, 1)
  function parseMicotexCOM3(raw) {
    const s = String(raw || '').replace(/[\x00-\x1F]/g, '');
    // căutăm „kg” (case-insensitive, dar preferăm litere mici conform FoxPro)
    let idxKG = s.lastIndexOf('kg');
    if (idxKG < 0) {
      const S = s.toUpperCase();
      idxKG = S.lastIndexOf('KG');
    }
    if (idxKG >= 0) {
      const start = Math.max(0, idxKG - 38);
      const chunk = s.substr(start, 6);
      const signChar = s.charAt(idxKG - 40);
      const semn = (signChar === '-') ? -1 : 1;

      let num = parseFloat(chunk.replace(/[^0-9,.-]/g, '').replace(',', '.'));
      if (!Number.isNaN(num)) return semn * num;

      // fallback mai robust într-o fereastră din stânga „KG”
      ...
      if (ints && ints.length) {
        ...
        if (!Number.isNaN(iv)) return iv / 1000; // ex. 590 -> 0.590
      }
    }
    // fallback global
  ...
  async function readOnceCOM3() {
    ...
    try {
      ...
      // 1) ENQ
      { const w = port.writable.getWriter(); await w.write(Uint8Array.from([0x05])); w.releaseLock(); }
      await sleep(200);
      ...
      // 2) DC1
      { const w2 = port.writable.getWriter(); await w2.write(Uint8Array.from([0x11])); w2.releaseLock(); }
      await sleep(200);
      ...
      console.log('[Micotex COM3] RAW HEX:', bytesToHex(raw));
      console.log('[Micotex COM3] RAW TXT:', JSON.stringify(text));

      const val = parseMicotexCOM3(text);
      if (val !== null && isFinite(val)) {
        $('#cantitate_de_adaugat_prod').val(Number(val).toFixed(3)).trigger('input').trigger('change');
      } else {
        alert('Micotex COM3: nu am putut extrage o greutate validă.');
      }
    } catch (err) {
      console.error('Eroare Web Serial Micotex COM3:', err);
      alert('Eroare Web Serial Micotex COM3: ' + err);
    } finally {
      try {
        try { const r = port?.readable?.getReader(); if (r) { await r.cancel(); r.releaseLock(); } } catch(_) {}
        try { await port?.close(); } catch(_) {}
  ...
  window.readScaleMicotexCOM3Once = readOnceCOM3; // dacă vrei să-l chemi din alt flux
})();
...
(function initMicotexCOM1() {
  ...
  async function readOnceCOM1() {
    ...
    try {
      ...
      const raw = new Uint8Array(chunks);
      const text = new TextDecoder('utf-8').decode(raw);
      console.log('[Micotex COM1] RAW HEX:', bytesToHex(raw));
...
async function processBarcode() {
    ...
    const addWithDetails = (p, qty) => {
        ...
        return $.get("vanzare_adaug_prod_pe_nota.php", productData, 'json');
    };
    const onDone = (response) => { processAddProductResponse(response); finalizeAndResetInputs(); };
    ...
    try {
        ...
        if (productCache[codBare]) {
            ...
            return $.get("vanzare_adaug_prod_pe_nota.php", {
...
$('#text-keyboard-modal').on('click', '.key', function() {
  ...
  // alternativ: const key = String($(this).data('key') ?? '');
  ...
  if (action) {
    ...
    // Dezactivează Shift după o literă apăsată (poți lăsa așa)
...
(function initTotalHoverPopup(){
  ...
  $(document)
    .on('mouseenter', '#total_de_incasat_display', function(){
      ...
      // Dacă vrei cu “RON” lângă, folosește `${val} RON` în loc de val:
...
<script>
document.getElementById("btnExportOffline").onclick = function() {
    document.getElementById("exportModal").style.display = "block";
};

document.querySelector(".close-export").onclick = function() {
