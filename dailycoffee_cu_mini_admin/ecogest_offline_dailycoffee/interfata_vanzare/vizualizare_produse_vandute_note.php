...
$locatie_activa = (int)($app_cod_locatie ?? $_SESSION['cod_locatie'] ?? 2);
...
$min_datetime_sql = "
    SELECT 
        MIN(CAST(CONCAT($tabel_final_note.data_bon, ' ', $tabel_final_note.ora_bon) AS DATETIME)) AS min_dt,
        MAX(CAST(CONCAT($tabel_final_note.data_bon, ' ', $tabel_final_note.ora_bon) AS DATETIME)) AS max_dt
    ...
    WHERE $tabel_final_note.cod_inchidere = 0 
      ...
      AND $tabel_final_note.locatie = :cod_locatie";
$min_datetime_stmt = $pdo->prepare($min_datetime_sql);
$min_datetime_stmt->execute([':adm_id' => $adm_id, ':cod_locatie' => $locatie_activa]);
...
$f_stmt->execute([':adm_id' => $adm_id, ':cod_locatie' => $locatie_activa]);
...
$f_total_stmt->execute([':adm_id' => $adm_id, ':cod_locatie' => $locatie_activa]);
