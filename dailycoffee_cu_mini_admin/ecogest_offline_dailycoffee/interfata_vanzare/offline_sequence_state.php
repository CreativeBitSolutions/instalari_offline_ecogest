...
function offline_close_shift(PDO $pdo, string $notesTable, string $closuresTable, int $location, int $operator, string $date, string $time): int
{
    ...
    try {
        ...
        $closureIdStmt = $pdo->prepare("SELECT id_inch FROM {$closuresTable} WHERE cod_inchidere = ? AND CAST(COALESCE(NULLIF(cod_locatie, 0), locatie, 0) AS INTEGER) = ? ORDER BY id_inch DESC LIMIT 1");
        $closureIdStmt->execute([$code, $location]);
        $closureId = (int)$closureIdStmt->fetchColumn();
        if ($closureId > 0) {
            offline_sync_enqueue_closure_safely($pdo, $closureId);
        }
...
function offline_close_z_report(PDO $pdo, int $location, int $reportNumber, array $closureCodes, array $payments, string $dateTime): void
{
    ...
    try {
        ...
        if ($reportId <= 0) {
            $reportIdStmt = $pdo->prepare("SELECT id FROM rapoarte_z WHERE nr_raport_z = ? AND cod_locatie = ? AND COALESCE(serie_casa_marcat, '') = ? AND COALESCE(nui, 0) = ? AND COALESCE(serie_memorie_fiscala, '') = ? ORDER BY id DESC LIMIT 1");
            $reportIdStmt->execute([$reportNumber, $location, $series, $nui, $memory]);
        ...
        if ($reportId > 0) {
            offline_sync_enqueue_report_safely($pdo, $reportId);
        }
