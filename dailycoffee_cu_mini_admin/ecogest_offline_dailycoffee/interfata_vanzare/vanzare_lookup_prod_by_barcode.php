...
try {
    ...
    if (!$produs_info) {
        $stmt = $pdo->prepare("SELECT cod_produs FROM produse_servicii n
            WHERE n.cod_bare = :cod_bare
              AND n.activ = 1
              AND EXISTS (
            ...
            LIMIT 1");
        $stmt->execute([':cod_bare' => $barcodeRaw, ':cod_locatie' => (int)$cod_locatie]);
        ...
        $sql = "SELECT n.cod_produs, n.nume, n.pret_cu_tva, n.cota_tva, n.um,
                ...
                LIMIT 1";
        $st = $pdo->prepare($sql);
        $st->execute([':cod_p' => $cod_p, ':cod_locatie' => (int)$cod_locatie]);
    ...
    $availability->execute([
        ':cod_produs' => (int)$produs_info['cod_produs'],
        ':cod_locatie' => (int)$cod_locatie,
    ]);
    if (!$availability->fetchColumn()) {
        http_response_code(404);
        echo json_encode(['error' => 'Produs indisponibil pentru locatia configurata.']);
        exit;
    }
