...
if (!$nir_data) {
    ...
    try {
        ...
        $produs = (int)($_POST['cod_produs'] ?? 0); // Codul produsului selectat
        $cod_locatie_nir = (int)($nir_data['cod_locatie'] ?? $_SESSION['cod_locatie'] ?? 2);
        if ($produs <= 0 || $cod_locatie_nir <= 0) {
            throw new Exception("Produsul sau locatia NIR nu sunt valide.");
        }

        $availability = $pdo->prepare("SELECT 1
            ...
            WHERE ps.cod_produs = :cod_produs
              ...
              AND EXISTS (
                  ...
                  WHERE psl.cod_produs = ps.cod_produs
        ...
        if (!$availability->fetchColumn()) {
            throw new Exception("Produsul nu este activ pentru locatia NIR configurata.");
        }
...
$syncNirStmt = $pdo->prepare('SELECT id_nir FROM nir WHERE nr_nir = ? LIMIT 1');
