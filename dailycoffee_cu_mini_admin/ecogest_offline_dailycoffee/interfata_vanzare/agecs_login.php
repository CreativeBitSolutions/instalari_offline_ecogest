...
$dsqlParams = [':locatie' => $cod_locatie];
if (($_SESSION['d'] ?? 0) == 1) {
                $dsql = "SELECT * FROM $tabel_admins WHERE locatie=:locatie AND lucreaza_la='magazin'";
    } else {
                $dsql = "SELECT * FROM $tabel_admins WHERE locatie=:locatie AND lucreaza_la='magazin'";
    }

    if ((int)($_SESSION['client_id'] ?? 0) === 2 && (int)$cod_locatie === 2) {
        $dsql .= " AND admin_email_address=:dailycoffee_offline_operator_email";
        $dsqlParams[':dailycoffee_offline_operator_email'] = 'offline.c2.l2.o2@agecs.local';
    }
    

$dstmt = $pdo->prepare($dsql);
$dstmt->execute($dsqlParams); 
...
?>
    ...
    <div class="buttons">
        ...
        <?php if ((int)($_SESSION['client_id'] ?? 0) === 2 && (int)($_SESSION['cod_locatie'] ?? 0) === 2): ?>
            <div class="products-sync-notice">
                <span>Catalogul complet se pastreaza local. La vanzare si la NIR sunt afisate doar produsele active pentru locatia 2.</span>
                <a class="products-sync-btn" href="offline_products_check.php" title="Preia catalogul complet de produse si categorii din online">ACTUALIZEAZA PRODUSE DIN ONLINE</a>
            </div>
        <?php endif; ?>
