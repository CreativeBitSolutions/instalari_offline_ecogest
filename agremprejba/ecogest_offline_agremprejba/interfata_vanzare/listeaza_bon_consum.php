...
$nr_bc=$_SESSION['nr_doc'];
$factsql = "SELECT * from $tabel_final_miscari inner join $tabel_final_nomenclator on $tabel_final_miscari.cod_p=$tabel_final_nomenclator.cod_p where $tabel_final_miscari.nr_doc='$nr_bc' and $tabel_final_miscari.fel_doc='BCF' group by $tabel_final_miscari.nr_doc";
$factstmt = $pdo->prepare($factsql);  
$factstmt->execute(); 
while ($row = $factstmt->fetch(PDO::FETCH_ASSOC)){
    $data_bon=$row['data'];
$nr_nota=$row['nr_nota'];
$produs_obtinut=$row['produs_obtinut'];
          
} 

$factsql2 = "SELECT * from $tabel_final_nomenclator where cod_p='$produs_obtinut' ";
$factstmt2 = $pdo->prepare($factsql2);  
$factstmt2->execute(); 
while ($row = $factstmt2->fetch(PDO::FETCH_ASSOC)){
	$den_p=$row['den_p'];
}
...
$pdf->Cell(10	,5,'Gestiunea predatoare:',0,0);
$pdf->Cell(120	,5,'',0,0);
...
$pdf->Cell(34	,5,$nr_nota,0,1);//end of line
$pdf->Cell(10	,5,'',0,0);
$pdf->Cell(120	,5,'',0,0);
...
$Y= 93;
$factsql = "SELECT * from $tabel_final_miscari inner join $tabel_final_nomenclator on $tabel_final_miscari.cod_p=$tabel_final_nomenclator.cod_p where $tabel_final_miscari.nr_doc='$nr_bc' and $tabel_final_miscari.fel_doc='BCF' ";
$factstmt = $pdo->prepare($factsql);  
$factstmt->execute(); 
while ($row = $factstmt->fetch(PDO::FETCH_ASSOC)){
