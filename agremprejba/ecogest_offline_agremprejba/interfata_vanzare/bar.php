...
$ora_prep_bon = date("H:i:s", strtotime('+0 hours'));
 $data_prep_bon = date("Y-m-d", strtotime('+0 hours'));
	...
	if(!isset($_SESSION['nr_bon'])){
	    $ccom_sql = "SELECT $tabel_final_de_listat_bar.nr_bon,$tabel_final_de_listat_bar.ora,$tabel_final_note.cod_masa FROM $tabel_final_de_listat_bar inner join $tabel_final_note on $tabel_final_de_listat_bar.nr_bon=$tabel_final_note.nrbon  where $tabel_final_de_listat_bar.preparat=0 group by nr_bon desc";    
...
$_SESSION['masa_curenta']=$row['cod_masa'];
    ...
    <div class="tab" style='margin:0'>
        <button disabled style="float:left" class="tablinks" >Interfata Bar</button>
...
//afisare randuri 
$f_sql = "SELECT $tabel_final_de_listat_bar.id,$tabel_final_de_listat_bar.id_vanz,$tabel_final_de_listat_bar.nr_bon,$tabel_final_de_listat_bar.cantitate,$tabel_final_de_listat_bar.pachet,$tabel_final_de_listat_bar.preparat,$tabel_final_nomenclator.den_p,$tabel_final_de_listat_bar.data_com,$tabel_final_de_listat_bar.ora_com,$tabel_final_de_listat_bar.data_prep,$tabel_final_de_listat_bar.ora_prep FROM $tabel_final_de_listat_bar INNER JOIN $tabel_final_nomenclator on $tabel_final_de_listat_bar.cod_p=$tabel_final_nomenclator.cod_p where $tabel_final_de_listat_bar.nr_bon='$nr_bon' order by $tabel_final_de_listat_bar.id;";   
...
$prep=$row['preparat'];
  ...
  if (isset($_POST[$id_simplu_vanz])) {
					$sterg_sql="UPDATE $tabel_final_de_listat_bar set preparat='1',ora_prep='$ora_prep_bon',data_prep='$data_prep_bon' where id='$id_vanz'";
...
$mese_desch_sql = "SELECT $tabel_final_note.cod_masa,$tabel_final_de_listat_bar.nr_bon FROM $tabel_final_de_listat_bar inner join $tabel_final_note on $tabel_final_de_listat_bar.nr_bon=$tabel_final_note.nrbon where $tabel_final_de_listat_bar.preparat='0' group by $tabel_final_de_listat_bar.nr_bon";    
...
<button title='Mentineti apasat pentru a derula in jos' class='btn-sm scrol2 hidden-xs' id ='scrolldown2'><i class='fa fa-chevron-down'></i></button>   
            ...
            <?php 
$mese_desch_sql = "SELECT $tabel_final_note.cod_masa,$tabel_final_de_listat_bar.nr_bon FROM $tabel_final_de_listat_bar inner join $tabel_final_note on $tabel_final_de_listat_bar.nr_bon=$tabel_final_note.nrbon group by $tabel_final_de_listat_bar.nr_bon";    
$mese_desch_stmt = $pdo->prepare($mese_desch_sql);  
$mese_desch_stmt->execute(); 
...
$ttest_sql = "SELECT sum($tabel_final_de_listat_bar.cantitate) as total_cant_bon FROM $tabel_final_de_listat_bar where $tabel_final_de_listat_bar.nr_bon='$nota' and $tabel_final_de_listat_bar.preparat='0'";
