...
<script>
  ...
  $(function(){
    ...
    $(document).on('click','button.obs',function(){
      var idVanzare = $(this).attr('name');
      var observatieExistenta = $(this).data('observatie') || '';

      $('#idvanzare_obs').val(idVanzare);
      $('#observatie_produsInput').val(observatieExistenta);
...
?>
  ...
  <?php if (!function_exists('restaurantIsOfflineSqlite') || !restaurantIsOfflineSqlite()): ?>
  (function(){
  ...
  $(function(){
    ...
    $(document).on('click','.btn-prioritate-step, .btn-prioritate', function(e){
      e.preventDefault();

      const button = $(this);
      const idVanz = parseInt(button.data('idvanz'), 10) || 0;
      const currentPriority = parseInt(button.data('current-priority'), 10) || 0;
      const stepAttr = button.data('step');
      const step = (stepAttr === undefined || stepAttr === null || stepAttr === '') ? 1 : (parseInt(stepAttr, 10) || 0);
      const newPriority = currentPriority + step;
      ...
      button.closest('.btn-prioritate-group').find('button').prop('disabled', true);
      ...
      $.ajax({
        ...
        if (r.success){
          ...
          button.closest('.btn-prioritate-group').find('button').prop('disabled', false);
        }
      }).fail(function(){
  ...
  var CIF_CLIENT_LIMIT = 500;
  var CIF_CLIENT_LIMIT_MESSAGE = 'Valoarea bonului depaseste 500 lei si ai completat CIF-ul clientului. Pentru aceasta tranzactie trebuie emisa si factura fiscala. Daca facturarea automata este configurata, factura va fi generata automat. In caz contrar, factura trebuie creata manual. Doresti sa continui incasarea? OK = DA, Cancel = NU.';

  function parseCifLimitAmount(value) {
    value = (value || '').toString().replace(/[^\d.,-]/g, '').replace(',', '.');
    var number = parseFloat(value);
    return isNaN(number) ? 0 : Math.round(number * 100) / 100;
  }

  function readCifLimitValue(selector) {
    var element = document.querySelector(selector);
    if (!element) return '';
    return element.value || element.getAttribute('data-total-cu-tva') || element.textContent || '';
  }

  function currentBonTotalForCifLimit(methodValue) {
    if (methodValue === 'numerar_si_card') {
      return parseCifLimitAmount(readCifLimitValue('#mixt_total'));
    }
    if (methodValue === 'tichete_de_masa') {
      return parseCifLimitAmount(readCifLimitValue('#total_de_plata_tichete'));
    }
    return parseCifLimitAmount(
      readCifLimitValue('#total') ||
      readCifLimitValue('#total_cu_tva') ||
      readCifLimitValue('[data-total-cu-tva]') ||
      readCifLimitValue('#mixt_total') ||
      readCifLimitValue('#total_de_plata_tichete')
    );
  }

  function currentCifForLimit(methodValue) {
    var selector = '#cif_client, [name="cif_client"], #cif_client_m, [name="cif_client_m"], #cif_client_t, [name="cif_client_t"]';
    if (methodValue === 'numerar_si_card') {
      selector = '#cif_client_m, [name="cif_client_m"], #cif_client, [name="cif_client"]';
    } else if (methodValue === 'tichete_de_masa') {
      selector = '#cif_client_t, [name="cif_client_t"], #cif_client, [name="cif_client"]';
    }
  ...
  function enforceCifClientLimit(methodValue) {
    if (methodValue === 'virament_bancar_separat_fara_casa_marcat') return true;
    if (!currentCifForLimit(methodValue)) return true;
    if (currentBonTotalForCifLimit(methodValue) <= CIF_CLIENT_LIMIT) return true;

    var confirmed = window.confirm(CIF_CLIENT_LIMIT_MESSAGE);
    var loading = document.getElementById('loading');
    if (!confirmed && loading) loading.style.display = 'none';
    return confirmed;
  }

  function postToProcesare(methodValue){
    try{
      if (!enforceCifClientLimit(methodValue)) return;
...
<?php if ((!function_exists('restaurantIsOfflineSqlite') || !restaurantIsOfflineSqlite()) && in_array((int)($_SESSION['client_id'] ?? 0), [25, 26], true)): ?>
<script>
(function ($) {
  if (!$) return;
