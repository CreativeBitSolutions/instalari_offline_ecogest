...
$offlineWooEnabled = $offlineMode && in_array((int)($_SESSION['client_id'] ?? 0), [25, 26], true);
$wooNotifAudioSrc = (!$offlineMode || $offlineWooEnabled) ? 'woo_comanda_noua.mpeg' : '';
...
<div class="page-container">
  ...
  <button id="scroll-cat-right" class="scroll-btn"><i class="fas fa-chevron-right"></i></button>
    ...
    <div class="quick-actions-box mb-2">
        ...
        <?php if (!$offlineMode || $offlineWooEnabled): ?>
    ...
    <?php endif; ?>
        ...
        <div id="white_button" class="banner1 action-btn">
          ...
          ?></div>
          <?php if ($offlineMode): ?>
          <div><strong>2FA tabletă:</strong> <span id="operator2faCodeMain">N/A</span>
            <button type="button" id="btnRegenerate2FAHeader" class="btn btn-sm btn-outline-primary ml-2">Generează cod nou</button>
          </div>
          <?php endif; ?>
...
<?php if (!$offlineMode) include('vanzare_modal_bonanswer_erori.php'); ?>
  ...
  <?php
    $wooPopupContext = 'sales';
    include('woo_notification_popup.php');
  ?>
...
(function($){
  ...
  function requestTwoFa(action, showError){
    var $button = $('#btnRegenerate2FAHeader');
    if(action === 'regenerate') $button.prop('disabled', true).text('Se generează...');
    $.ajax({url:'vanzare_tableta_2fa_api.php',type:'POST',dataType:'json',cache:false,data:{action:action}})
      .done(function(response){
        if(response && response.status === 'success'){
          $('#operator2faCodeMain').text(response.active === false || !response.code ? 'N/A' : response.code);
        } else if(showError){
          alert(response && response.message ? response.message : 'Nu s-a putut genera codul 2FA.');
        }
      })
      .fail(function(xhr){
        if(showError) alert(xhr && xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Eroare de comunicare la generarea codului 2FA.');
      })
      .always(function(){ if(action === 'regenerate') $button.prop('disabled', false).text('Generează cod nou'); });
  }
  $(document).on('click','#btnRegenerate2FAHeader',function(){requestTwoFa('regenerate',true);});
  $(function(){requestTwoFa('status',false);});
})(window.jQuery);
</script>
<?php endif; ?>
