...
<?php $wooPopupContext = 'login'; require __DIR__ . '/woo_notification_popup.php'; ?>
...
(function () {
    var inFlight = false;
    function checkWooOrders() {
        if (inFlight) return;
        inFlight = true;
        fetch('woo_check_comenzi_noi.php?source=login&ts=' + Date.now(), {credentials:'same-origin',cache:'no-store',headers:{'Accept':'application/json'}})
            .then(function (response) { if (!response.ok) throw new Error('HTTP ' + response.status); return response.json(); })
            .then(function (data) { if (data && data.success === true && window.agecsWooOrderPopup) window.agecsWooOrderPopup.update(data.count, data.ids); })
            .catch(function (error) { if (window.console && console.warn) console.warn('Verificarea comenzilor de pe site a eșuat:', error); })
            .finally(function () { inFlight = false; });
    }
    checkWooOrders();
    setInterval(checkWooOrders, 20000);
})();
</script>
<?php endif; ?>
<script>
