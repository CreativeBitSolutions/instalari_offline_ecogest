        ...
        // ✅ MODIFICARE: Am adăugat `dn.prioritate` în SELECT și am ordonat rezultatele
            $products_sql = "
                ...
                WHERE dn.nr_bon = :nrbon
                  ...
                  AND $filter_t_list
                ORDER BY dn.prioritate, dn.id_vanz
            ";
...
$total_nota = 0;
            ...
            $groupedProducts = [];
            if ($client_id == 9 || $client_id == 8) {
                // Pentru client_id 9 și 8 NU grupăm deloc, păstrăm ordinea de adăugare
            ...
            // ✅ MODIFICARE: Distribuim produsele în categorii de prioritate
            $produsePeFeluri = [
                1 => [], // FEL 1
                2 => [], // FEL 2
                3 => [], // FEL 3
                0 => []  // Fără prioritate (diverse)
            ];

            foreach ($groupedProducts as $product) {
                $prioritate = (int)$product['prioritate'];
                if (isset($produsePeFeluri[$prioritate])) {
                    $produsePeFeluri[$prioritate][] = $product;
                } else {
                    // Orice altă valoare (improbabil, dar sigur) ajunge la diverse
                    $produsePeFeluri[0][] = $product;
                }
            }
            ...
            $formatProductLine = function($product, $client_id, $separator) use ($departament_listare) {
                ...
                $hide_price = (strpos($dept_upper, 'BUCATARIE') !== false || 
            ...
            // Adăugăm produsele cu prioritate 0, FĂRĂ titlu de categorie
            if (!empty($produsePeFeluri[0])) {
                foreach ($produsePeFeluri[0] as $product) {
                    $continut .= $formatProductLine($product, $client_id, $separator);
                    $total_nota += $product['valoare_vanzare_cu_tva'];
                }
            }
