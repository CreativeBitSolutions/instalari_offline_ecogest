...
?>
<!doctype html><html lang="ro"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Comenzi WooCommerce - Grand Plaza</title><link rel="stylesheet" href="vendor/bootstrap/css/bootstrap.min.css"><style>body{background:#f5f6f8}.head{background:#1f2937;color:#fff}.card{border:0;box-shadow:0 2px 8px #0001}.map{border-left:5px solid #f0ad4e}.order{border-left:5px solid #0d6efd}.done{border-left:5px solid #198754}.muted{font-size:.85rem;color:#6c757d}.items{font-size:.92rem}</style><?php include __DIR__ . '/i18n/i18n_bootstrap.php'; ?></head><body>
<div class="head p-3"><div class="container-fluid d-flex justify-content-between align-items-center"><div><h3 class="mb-1">Comenzi WooCommerce - Grand Plaza</h3><small><?=wooOfflineH($cfg['base_url']??'')?> · client <?=wooOfflineH($cfg['client_id']??'')?> / locație <?=wooOfflineH($cfg['location_id']??'')?></small></div><a href="vanzare_restaurant.php" class="btn btn-light">Înapoi la vânzare</a></div></div>
...
<h5>Comenzi preluate</h5><div class="card done"><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>Woo</th><th>Nota POS</th><th>Importat</th><th>Sursă</th><th>ACK site</th><th></th></tr></thead><tbody><?php foreach($imported as $row):$importSource=(string)($row['import_source']??'offline');?><tr><td>#<?=wooOfflineH($row['order_number']?:$row['woo_order_id'])?></td><td><?=(int)$row['imported_note_nrbon']?></td><td><?=wooOfflineH($row['imported_at'])?></td><td><?=$importSource==='online'?'AGECS online':'Instalare offline'?></td><td><?=$importSource==='online'?'Nu se retrimite':wooOfflineH($row['ack_status'])?><?php if($importSource!=='online'&&trim((string)$row['ack_error'])!==''):?><div class="small text-danger"><?=wooOfflineH($row['ack_error'])?></div><?php endif;?></td><td><?php if($importSource!=='online'&&$row['ack_status']!=='acknowledged'):?><form method="post"><input type="hidden" name="action" value="retry_ack"><input type="hidden" name="woo_order_id" value="<?=wooOfflineH($row['woo_order_id'])?>"><input type="hidden" name="note_id" value="<?=(int)$row['imported_note_nrbon']?>"><button class="btn btn-sm btn-outline-primary">Retrimite ACK</button></form><?php endif;?></td></tr><?php endforeach;?></tbody></table></div></div>
</div></body></html>
