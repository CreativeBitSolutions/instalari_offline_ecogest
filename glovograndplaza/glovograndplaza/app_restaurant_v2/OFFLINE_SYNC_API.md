...
Notele cu status `S` rămân exclusiv în SQLite. O notă finalizată este pusă imediat în coadă, chiar dacă nu are încă `cod_inchidere` sau `nr_raport_z`. Închiderea turei și raportul Z sunt sincronizate ulterior ca actualizări independente.
...
Transmiterea manuală poate fi pornită numai de un operator cu rang `sefsala`, `administrator` sau `admin`. Workerul automat rulează și în interfața ospătarului și în pagina de alegere a operatorului.
